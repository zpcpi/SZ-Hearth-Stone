...
local mri = cPublications
mri.m_cConfig.m_bAllMemoryRefFileAddTime = false



local sFilePath = "D:/LuaSnapshot/"
function GF.api.BeginSnaoshot()
	collectgarbage("collect")
	mri.m_cMethods.DumpMemorySnapshot(sFilePath, "1-Before", -1)
end


function GF.api.EndSnaoshot()
	collectgarbage("collect")
	mri.m_cMethods.DumpMemorySnapshot(sFilePath, "2-After", -1)
	mri.m_cMethods.DumpMemorySnapshotComparedFile(sFilePath, "Compared", -1, sFilePath.."LuaMemRefInfo-All-[1-Before].txt", sFilePath.."LuaMemRefInfo-All-[2-After].txt")
	print("Dump memory snapshot information finished! ")
end

function GF.api.collectgarbage()
    print("old mb:",collectgarbage("count") / 1024)
    collectgarbage("collect")
    print("now mb:",collectgarbage("count") / 1024)
end

function GF.api.collectgarbageCount()
   -- oldPrint("now mb:",collectgarbage("count") / 1024)
   print("now mb:",collectgarbage("count") / 1024)
end
