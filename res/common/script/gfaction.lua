...
table.insert(GF.update, function(t)
	if GF.cacheData and GF.cacheData.animIsPlaying then 
		if GF.cacheData.animIsPlaying_time_attack and os.clock() - GF.cacheData.animIsPlaying_time_attack > 5 then -- 卡在动画5秒
			GF.action_trigevent('动画事件_触发攻击帧2')
			GF.cacheData.animIsPlaying_time_attack = nil
		end
		if GF.cacheData.animIsPlaying_time_anim and os.clock() - GF.cacheData.animIsPlaying_time_anim > 30 then -- 卡在动画30秒
			GF.trig_event('所有动画播放结束')
			GF.cacheData.animIsPlaying_time_anim = nil
		end
	end
	tick_timer(t)
	es.tick(clED)
...
function GF.PlayAnim(node, anim, loop, time)
	...
	if node and node.type == "SpineQuad" then
		if GF.GetSpineAnimTime == nil then 
			return node.playAnim(anim, loop, time)
		else
			node.playAnim(anim, loop, time)
		end
		return GF.GetSpineAnimTime(node.animData,anim)
	end
...
local EMPTYTABLE = {}
function GF.AnimationEventTime(spine_name,anim_name,event_name)
	local spineInfo = require('spineAnimInfo')
	return ((spineInfo[spine_name] or EMPTYTABLE)[anim_name] or EMPTYTABLE)[event_name] or 0
end
...
function GF.action_wait1(func,...)
	...
	if func == '动画事件_触发攻击帧2' then 
		GF.cacheData.animIsPlaying_time_attack = os.clock()
	end
...
function GF.ClearAction()
	...
	timer = {}
	GF.__timer = timer
	callback = setmetatable({}, co.weak_metak)
end