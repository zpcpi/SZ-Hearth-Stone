...
table.insert(GF.update, function(t)
	...
	if GF.needSaveCount == 0 then
		if GF.GetCurScript() then 
			local readState = GF.get_readonly()
			GF.set_readonly(false)
			GF.call('杂项_存档')
			GF.set_readonly(readState)
		end
		GF.needSave = false
	end
...
local tab_wait_case_ = {
	['角色交互'] = 'wait_case_角色交互',['选择角色'] = 'wait_case_选择角色',
	['选择场景'] = 'wait_case_选择场景',['新迷宫事件_点击任务事件卡片'] = 'wait_case_新迷宫事件_点击任务事件卡片',
}

local tab_wait_case_end_ = {
	['角色交互'] = 'wait_case_end_角色交互',['选择角色'] = 'wait_case_end_选择角色',['选择场景'] = 'wait_case_end_选择场景',
	['新迷宫事件_点击任务事件卡片'] = 'wait_case_end_新迷宫事件_点击任务事件卡片',
}
...
function GF.wait1(arg,...)
	...
	if func == '等待下一帧' or func == '任务等待下一帧' then
		GF.wait_time(20)
		return
	end
	if func == '所有动画播放结束' then 
		GF.cacheData.animIsPlaying_time_anim = os.clock()
	end
	...
	if noti_func == '角色交互' or noti_func == '选择角色' or noti_func == '选择场景' or noti_func == '新迷宫事件_点击任务事件卡片' then
		if ANIMATION_NOT_LIST then
			GF.call(tab_wait_case_[noti_func],earg)
		else
			GF.call('添加到动画序列',{'动画_动画回调',function()
				GF.call(tab_wait_case_[noti_func],earg)
			end})
		end
	end
	...
	if noti_func == '角色交互' or noti_func == '选择角色' or noti_func == '选择场景' or noti_func == '新迷宫事件_点击任务事件卡片' then
		if ANIMATION_NOT_LIST then
			GF.call(tab_wait_case_end_[noti_func],earg)
		else
			oldPrint(noti_func)
			GF.call('添加到动画序列',{'动画_动画回调',function()
				GF.call(tab_wait_case_end_[noti_func],earg)
...
function GF.wait_case()
	...
	for k = 1,#arg do
		local func = arg[k][1]
		if func == '角色交互' or func == '选择角色' or func == '选择场景' or func == '新迷宫事件_点击任务事件卡片' then
			GF.call(tab_wait_case_[func],arg[k])
		end
	...
	for k = 1,#arg do
		local func = arg[k][1]
		if func == '角色交互' or func == '选择角色' or func == '选择场景' or func == '新迷宫事件_点击任务事件卡片' then
			GF.call(tab_wait_case_end_[func],arg[k])
		end
...
_G.remote_event = function(buf)
	...
	GF.trig_event(table_unpack(arg))
end
...
function GF.trig_event(...)
	-- GF.log('gfcall trig_event',...)
	-- print('gfcall trig_event',...)
