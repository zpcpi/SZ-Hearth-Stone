--[[1006

]]
local t = {
'o_card',
{
	{
		['name']=0x10060003,
		['showname']='月火术',
		['卡牌属性']={
			['费用']=0,
			['过载费用']=0,
		},
		['美术数据']={
			['原画']=0x56100002,
			['画家']='Richard Wright',
			['描述']='造成1点伤害。',
			['趣味描述']='“把月火术拖到每一个技能栏位上。”——《如何成为一名德鲁伊》，第五章，第三节',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130001,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=467,
			['uid']='CS2_008',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060004,
		['showname']='激活',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x5610000a,
			['画家']='Doug Alexander',
			['描述']='在本回合中，获得一个法力水晶。',
			['趣味描述']='有些德鲁伊做梦时都回响着陌生人的喊叫声：“给我个激活！”',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130002,
			},
		},
		['局外数据']={
			['dbfid']=254,
			['uid']='EX1_169',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060005,
		['showname']='爪击',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56100003,
			['画家']='Dany Orizio',
			['描述']='使你的英雄获得2点护甲值，并在本回合中获得+2攻击力。',
			['趣味描述']='谁去谁留，都由爪子决定。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130003,
			},
		},
		['局外数据']={
			['dbfid']=1050,
			['uid']='CS2_005',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060006,
		['showname']='野性印记',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56100006,
			['画家']='Brad Vancata',
			['描述']='使一个随从获得嘲讽和+2/+2。（+2攻击力/+2生命值）',
			['趣味描述']='这种爪子形状的印记如果纹在身上，人们一定知道你是德鲁伊玩家。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130004,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=213,
			['uid']='CS2_009',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060007,
		['showname']='治疗之触',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56100009,
			['画家']='Cyril Van Der Haegen',
			['描述']='恢复8点生命值。',
			['趣味描述']='见效快，疗效好，无毒副作用。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130006,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=773,
			['uid']='CS2_007',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060008,
		['showname']='野性成长',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56100005,
			['画家']='James Ryman',
			['描述']='获得一个空的法力水晶。',
			['趣味描述']='使用法力水晶增长装置增加你的法力值，只要399！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130008,
			},
		},
		['局外数据']={
			['dbfid']=1124,
			['uid']='CS2_013',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x1006000d,
			},
		},
	},
	{
		['name']=0x10060009,
		['showname']='野蛮咆哮',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56100001,
			['画家']='Grace Liu',
			['描述']='在本回合中，使你的所有角色获得+2攻击力。',
			['趣味描述']='显然这对火元素和水元素也有作用。难道这说明元素也是长耳朵的吗？',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130007,
			},
		},
		['局外数据']={
			['dbfid']=742,
			['uid']='CS2_011',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006000a,
		['showname']='横扫',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x56100004,
			['画家']='Sean O’Daniels',
			['描述']='对一个敌人造成4点伤害，并对所有其他敌人造成1点伤害。',
			['趣味描述']='当一只熊向后仰身张开它的前臂时，他就要横扫！...或者拥抱你了。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130009,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='敌方',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=64,
			['uid']='CS2_012',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006000b,
		['showname']='星火术',
		['卡牌属性']={
			['费用']=6,
		},
		['美术数据']={
			['原画']=0x56100007,
			['画家']='Alex Horley Orlandelli',
			['描述']='造成5点伤害。抽一张牌。',
			['趣味描述']='平衡对德鲁伊来说很重要。这张卡牌现在就很平衡了。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013000a,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=823,
			['uid']='EX1_173',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006000c,
		['showname']='埃隆巴克保护者',
		['卡牌属性']={
			['费用']=8,
			['攻击']=8,
			['生命']=8,
		},
		['美术数据']={
			['原画']=0x56100008,
			['画家']='Dave Allsop',
			['描述']='嘲讽',
			['趣味描述']='我打赌你一定不敢进攻达纳苏斯。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=205,
			['uid']='CS2_232',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006000d,
		['showname']='法力过剩',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56140001,
			['描述']='抽一张牌。（你最多可以拥有10个法力水晶。）',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013000c,
			},
		},
		['局外数据']={
			['dbfid']=1725,
			['uid']='CS2_013t',
		},
	},
	{
		['name']=0x10060074,
		['showname']='保底原画_法术卡',
		['postfix']='不会使用',
		['美术数据']={
			['原画']=0x56130002,
			['原画偏移Y']=22.0,
		},
		['逻辑数据']={
			['类型']=0x10090005,
		},
		['局外数据']={
			['可收集']=false,
		},
	},
	{
		['name']=0x10060075,
		['showname']='保底原画_随从卡',
		['postfix']='不会使用',
		['美术数据']={
			['原画']=0x56130002,
			['原画偏移X']=-8.0,
			['原画偏移Y']=10.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
		},
		['局外数据']={
			['可收集']=false,
		},
	},
	{
		['name']=0x10060076,
		['showname']='保底原画_武器卡',
		['postfix']='不会使用',
		['美术数据']={
			['原画']=0x56130002,
			['原画偏移Y']=18.0,
		},
		['逻辑数据']={
			['类型']=0x10090006,
		},
		['局外数据']={
			['可收集']=false,
		},
	},
	{
		['name']=0x10060077,
		['showname']='保底原画_英雄卡',
		['postfix']='不会使用',
		['美术数据']={
			['原画']=0x56130002,
			['原画偏移X']=-6.0,
			['原画偏移Y']=11.0,
		},
		['逻辑数据']={
			['类型']=0x10090002,
		},
		['局外数据']={
			['可收集']=false,
		},
	},
	{
		['name']=0x10060078,
		['showname']='保底原画_英雄技能',
		['postfix']='不会使用',
		['美术数据']={
			['原画']=0x56130002,
			['原画偏移X']=-4.0,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090003,
		},
		['局外数据']={
			['可收集']=false,
		},
	},
	{
		['name']=0x100600a7,
		['showname']='保底原画_英雄',
		['postfix']='不会使用',
		['卡牌属性']={
			['生命']=8,
		},
		['美术数据']={
			['原画']=0x56130002,
			['原画偏移Y']=4.0,
		},
		['逻辑数据']={
			['类型']=0x10090001,
		},
		['局外数据']={
			['可收集']=false,
		},
	},
	{
		['name']=0x10060079,
		['showname']='巫医',
		['卡牌属性']={
			['费用']=1,
			['攻击']=2,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270021,
			['原画偏移X']=-5.0,
			['画家']='Karl Richardson',
			['描述']='战吼：恢复2点生命值。',
			['趣味描述']='别以为带上大坏蛋面具，再把两根筷子插在嘴里，你就是巫医了，伙计。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040004,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130070,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='恢复2点生命值。',
			},
		},
		['局外数据']={
			['dbfid']=132,
			['uid']='EX1_011',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006000e,
		['showname']='幸运币',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56130001,
			['描述']='在本回合中，获得一个法力水晶。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013000b,
			},
		},
		['局外数据']={
			['dbfid']=1746,
			['uid']='GAME_005',
		},
	},
	{
		['name']=0x100600b5,
		['showname']='疲劳',
		['postfix']='公用',
		['卡牌属性']={
		},
		['美术数据']={
			['描述']='当你抽到该牌时，你的英雄受到伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['可收集']=false,
		},
	},
	{
		['name']=0x1006000f,
		['showname']='变形',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56140002,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]本回合+1攻击力。[br]+1护甲值。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['战场卡牌指令']=0x1004000c,
			['卡牌效果']={
				[1]=0x1013000d,
			},
		},
		['局外数据']={
			['dbfid']=1123,
			['uid']='HERO_06bp',
		},
	},
	{
		['name']=0x10060010,
		['showname']='奥术射击',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56110001,
			['画家']='Luca Zontini',
			['描述']='造成2点伤害。',
			['趣味描述']='法师们用奥术强化箭矢，然后出售给猎人；后来猎人们自己学会了足够的魔法之后便能够自给自足。不少法师因此失业，给暴风城的经济带来不小的冲击。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013000e,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=877,
			['uid']='DS1_185',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060011,
		['showname']='森林狼',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56110009,
			['原画偏移X']=-5.0,
			['画家']='Malcolm Davis',
			['描述']='你的其他野兽获得+1攻击力。',
			['趣味描述']='其他野兽最喜欢和森林狼一起外出打猎了。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x1013000f,
			},
		},
		['局外数据']={
			['dbfid']=606,
			['uid']='DS1_175',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060012,
		['showname']='追踪术',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x5611000a,
			['画家']='Mauro Cascioli',
			['描述']='检视你的牌库顶的三张牌，将其中一张置入手牌，弃掉其余牌。',
			['趣味描述']='为了那些不知道该把什么牌放入套牌的人准备的！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130010,
			},
		},
		['局外数据']={
			['dbfid']=1047,
			['uid']='DS1_184',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060013,
		['showname']='猎人印记',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56110006,
			['画家']='Jimmy Lo',
			['描述']='使一个随从的生命值变为1。',
			['趣味描述']='千万不要和猎人玩捉迷藏的游戏。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130011,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=141,
			['uid']='CS2_084',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060014,
		['showname']='动物伙伴',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56110005,
			['画家']='Wei Wang',
			['描述']='随机召唤一个野兽伙伴。',
			['趣味描述']='你可以召唤米莎，雷欧克或是霍弗！霍弗又没用，又爱惹麻烦！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌特性']={
				[1]=0,
				[2]=0,
				[3]=0,
				[4]=2,
			},
			['卡牌效果']={
				[1]=0x10130012,
			},
		},
		['局外数据']={
			['dbfid']=437,
			['uid']='NEW1_031',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x1006001b,
				[2]=0x1006001c,
				[3]=0x1006001d,
			},
		},
	},
	{
		['name']=0x10060015,
		['showname']='杀戮命令',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56110007,
			['画家']='Gabe from Penny Arcade',
			['描述']='造成3点伤害。如果你控制一个野兽，则改为造成5点伤害。',
			['趣味描述']='“咬死他！”，他下达了命令。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130014,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=296,
			['uid']='EX1_539',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060016,
		['showname']='多重射击',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x56110004,
			['画家']='Benjamin Zhang',
			['描述']='对两个随机敌方随从造成3点伤害。',
			['趣味描述']='看见没，我完全可以“一心两用”。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130015,
			},
		},
		['局外数据']={
			['dbfid']=292,
			['uid']='DS1_183',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060017,
		['showname']='驯兽师',
		['卡牌属性']={
			['费用']=4,
			['攻击']=4,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x56110003,
			['原画偏移X']=-5.0,
			['画家']='Dan Brereton',
			['描述']='战吼：使一个友方野兽获得+2/+2并获得嘲讽。',
			['趣味描述']='“是谁把狗放出来的？”他问。其实，这只是他的口头禅。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040004,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130017,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='我方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[5]={
						[1]=0x100a0006,
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='使一只野兽获得+2/+2并具有嘲讽。',
			},
		},
		['局外数据']={
			['dbfid']=1003,
			['uid']='DS1_070',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060018,
		['showname']='苔原犀牛',
		['卡牌属性']={
			['费用']=5,
			['攻击']=2,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x56110002,
			['原画偏移X']=-5.0,
			['画家']='Lars Grant-West',
			['描述']='你的野兽获得冲锋。',
			['趣味描述']='人们经常无法分清苔原犀牛和科多兽。这是一头苔原犀牛，对不对？',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x10130019,
			},
		},
		['局外数据']={
			['dbfid']=699,
			['uid']='DS1_178',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060019,
		['showname']='饥饿的秃鹫',
		['卡牌属性']={
			['费用']=5,
			['攻击']=3,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56110008,
			['原画偏移X']=-5.0,
			['画家']='Bernie Kang',
			['描述']='每当你召唤一个野兽，抽一张牌。',
			['趣味描述']='如果你喂饱了它，它就名不副实了。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x1013001a,
			},
		},
		['局外数据']={
			['dbfid']=1241,
			['uid']='CS2_237',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006001a,
		['showname']='稳固射击',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56160001,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]对敌方英雄造成2点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['战场卡牌指令']=0x1004000c,
			['卡牌效果']={
				[1]=0x1013001b,
			},
		},
		['局外数据']={
			['dbfid']=229,
			['uid']='HERO_05bp',
		},
	},
	{
		['name']=0x1006001b,
		['showname']='米莎',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=3,
			['攻击']=4,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x56160002,
			['原画偏移X']=-5.0,
			['描述']='嘲讽',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=2,
			},
		},
		['局外数据']={
			['dbfid']=959,
			['uid']='NEW1_032',
		},
	},
	{
		['name']=0x1006001c,
		['showname']='雷欧克',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=3,
			['攻击']=2,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x56160003,
			['原画偏移X']=-5.0,
			['描述']='你的其他随从获得+1攻击力。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=0,
				[3]=0,
				[4]=2,
			},
			['卡牌效果']={
				[1]=0x10130013,
			},
		},
		['局外数据']={
			['dbfid']=226,
			['uid']='NEW1_033',
		},
	},
	{
		['name']=0x1006001d,
		['showname']='霍弗',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=3,
			['攻击']=4,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56160004,
			['原画偏移X']=-5.0,
			['描述']='冲锋',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=1,
				[2]=0,
				[3]=0,
				[4]=2,
			},
		},
		['局外数据']={
			['dbfid']=100,
			['uid']='NEW1_034',
		},
	},
	{
		['name']=0x1006001e,
		['showname']='奥术飞弹',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56150005,
			['原画偏移X']=-4.0,
			['原画偏移Y']=10.0,
			['原画缩放X']=1,
			['原画缩放Y']=0.8,
			['画家']='Warren Mahy',
			['描述']='造成3点伤害，随机分配到所有敌人身上。',
			['趣味描述']='既然你想要做一个超级厉害的法师，就必须得把奥术飞弹控制得好一些。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013001c,
			},
		},
		['局外数据']={
			['dbfid']=564,
			['uid']='EX1_277',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006001f,
		['showname']='镜像',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x5615000b,
			['画家']='Jim Nelson',
			['描述']='召唤两个0/2，并具有嘲讽的随从。',
			['趣味描述']='哦，嘿，这是个镜像！！像镜个是这，嘿，哦',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013001d,
			},
		},
		['局外数据']={
			['dbfid']=1084,
			['uid']='CS2_027',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060020,
		['showname']='寒冰箭',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56150008,
			['画家']='Steve Ellis',
			['描述']='对一个角色造成3点伤害，并使其冻结。',
			['趣味描述']='在你用这张牌的时候，你可以随意喊出一些咒语，比如“冻住！”“冻结！”或者“结冰，结冰吧，宝贝儿！”',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013001e,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=662,
			['uid']='CS2_024',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060021,
		['showname']='魔爆术',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56150002,
			['画家']='Howard Lyon',
			['描述']='对所有敌方随从造成1点伤害。',
			['趣味描述']='魔爆术曾是让法师们区别于艾泽拉斯其他职业的技能之一。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013001f,
			},
		},
		['局外数据']={
			['dbfid']=447,
			['uid']='CS2_025',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060022,
		['showname']='冰霜新星',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56150007,
			['画家']='Josh Tallman',
			['描述']='冻结所有敌方随从。',
			['趣味描述']='嘿，伙计，这会冻死你的。我是说真的。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130020,
			},
		},
		['局外数据']={
			['dbfid']=587,
			['uid']='CS2_026',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060023,
		['showname']='奥术智慧',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56150006,
			['画家']='Dave Berggren',
			['描述']='抽两张牌。',
			['趣味描述']='使用这张牌能让你变得更聪明。面对现实吧：变聪明些对我们大家来说都是好事。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130021,
			},
		},
		['局外数据']={
			['dbfid']=555,
			['uid']='CS2_023',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060024,
		['showname']='变形术',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x56150009,
			['画家']='Vance Kovacs',
			['描述']='使一个随从变形成为1/1的绵羊。',
			['趣味描述']='你能想象一个嗜血的兽人发出咩咩的叫声吗？',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130022,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=77,
			['uid']='CS2_022',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060025,
		['showname']='水元素',
		['卡牌属性']={
			['费用']=4,
			['攻击']=3,
			['生命']=6,
		},
		['美术数据']={
			['原画']=0x56150004,
			['原画偏移X']=-5.0,
			['画家']='John Avon',
			['描述']='冻结任何受到该随从伤害的角色。',
			['趣味描述']='千万别在晚会上召唤水元素。它会把一切都搞砸的。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['种族']=0x100a0009,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x10130025,
			},
		},
		['局外数据']={
			['dbfid']=395,
			['uid']='CS2_033',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060026,
		['showname']='火球术',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x56150003,
			['画家']='Ralph Horsley',
			['描述']='造成6点伤害。',
			['趣味描述']='这个法术会让东西烧起来。如果你是在寻找能把东西烤熟，或者只是加热的法术，你就找错对象了。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130023,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=315,
			['uid']='CS2_029',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060027,
		['showname']='烈焰风暴',
		['卡牌属性']={
			['费用']=7,
		},
		['美术数据']={
			['原画']=0x5615000a,
			['画家']='Romain De Santi',
			['描述']='对所有敌方随从造成4点伤害。',
			['趣味描述']='当地板着火的时候，无论你在地上怎么打滚，都是没用的。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130024,
			},
		},
		['局外数据']={
			['dbfid']=1004,
			['uid']='CS2_032',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060028,
		['showname']='火焰冲击',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56180001,
			['原画偏移Y']=-12.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]造成1点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['战场卡牌指令']=0x1004000a,
			['卡牌效果']={
				[1]=0x1013002a,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='造成1点伤害。',
			},
		},
		['局外数据']={
			['dbfid']=807,
			['uid']='HERO_08bp',
		},
	},
	{
		['name']=0x10060029,
		['showname']='镜像',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=0,
			['攻击']=0,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56150001,
			['原画偏移X']=10,
			['原画偏移Y']=13.0,
			['描述']='嘲讽',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=968,
			['uid']='CS2_mirror',
		},
	},
	{
		['name']=0x1006002a,
		['showname']='保护之手',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56170008,
			['画家']='Clint Langley',
			['描述']='使一个随从获得圣盾。',
			['趣味描述']='这个法术的名字已经被修改了太多次，导致连圣骑士们都不知道它应该叫什么了。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130026,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=727,
			['uid']='EX1_371',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006002b,
		['showname']='力量祝福',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56170002,
			['画家']='Zoltan Boros',
			['描述']='使一个随从获得+3攻击力。',
			['趣味描述']='没有善，没有恶，也没有圣光。只有——力量！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130028,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=70,
			['uid']='CS2_087',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006002c,
		['showname']='圣光的正义',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x56170005,
			['原画缩放X']=0.9,
			['原画缩放Y']=0.9,
			['画家']='Glenn Rane',
			['趣味描述']='玛克扎尔王子收集了不少稀有的武器，他赋予它们生命，并让它们为他起舞。',
		},
		['逻辑数据']={
			['类型']=0x10090006,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=383,
			['uid']='CS2_091',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006002d,
		['showname']='谦逊',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56170004,
			['画家']='Daren Bader',
			['描述']='使一个随从的攻击力变为1。',
			['趣味描述']='这张牌真是会让人的心情很糟糕，就像潮湿阴冷的天气。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013002b,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=854,
			['uid']='EX1_360',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006002e,
		['showname']='圣光术',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56170001,
			['画家']='Zoltan & Gabor',
			['描述']='恢复6点生命值。',
			['趣味描述']='如果你经常暴露于圣光之下，你应该考虑随身带着防晒霜。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013002d,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=291,
			['uid']='CS2_089',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006002f,
		['showname']='奉献',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x56170006,
			['画家']='Vance Kovacs',
			['描述']='对所有敌人造成2点伤害。',
			['趣味描述']='神圣的能量从圣洁的大地涌出。但是，味道也有点熏人。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013002e,
			},
		},
		['局外数据']={
			['dbfid']=476,
			['uid']='CS2_093',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060030,
		['showname']='愤怒之锤',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x5617000b,
			['画家']='Efrem Palacios',
			['描述']='造成3点伤害。抽一张牌。',
			['趣味描述']='一个称职的圣骑士身上会携带很多工具。愤怒之锤、复仇之钳、正义之锯，等等。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013002f,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=250,
			['uid']='CS2_094',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060031,
		['showname']='王者祝福',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x56170007,
			['画家']='Lucas Graciano',
			['描述']='使一个随从获得+4/+4。（+4攻击力/+4生命值）',
			['趣味描述']='你知道世界上有多少所谓的“王者”被暗杀了吗？你确定还想要王者祝福吗？',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130030,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=943,
			['uid']='CS2_092',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060032,
		['showname']='真银圣剑',
		['卡牌属性']={
			['费用']=4,
			['攻击']=4,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56170009,
			['原画缩放X']=0.9,
			['原画缩放Y']=0.9,
			['画家']='Ryan Sook',
			['描述']='每当你的英雄进攻，便为其恢复2点生命值。',
			['趣味描述']='它能用来切片，也能切块。你还可以用它来开罐头。（但你不会这么做的。）',
		},
		['逻辑数据']={
			['类型']=0x10090006,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130032,
			},
		},
		['局外数据']={
			['dbfid']=847,
			['uid']='CS2_097',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060033,
		['showname']='列王守卫',
		['卡牌属性']={
			['费用']=7,
			['攻击']=5,
			['生命']=6,
		},
		['美术数据']={
			['原画']=0x56170003,
			['原画偏移X']=-5.0,
			['画家']='E.M. Gist',
			['描述']='战吼：为你的英雄恢复6点生命值。',
			['趣味描述']='神圣的造物从天而降...真是老掉牙的剧情！',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130033,
			},
		},
		['局外数据']={
			['dbfid']=1068,
			['uid']='CS2_088',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060034,
		['showname']='援军',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56190001,
			['原画偏移Y']=-8.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]召唤一个1/1的白银之手新兵。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['战场卡牌指令']=0x1004000c,
			['卡牌效果']={
				[1]=0x10130034,
			},
		},
		['局外数据']={
			['dbfid']=472,
			['uid']='HERO_04bp',
		},
	},
	{
		['name']=0x10060035,
		['showname']='白银之手新兵',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56190002,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=0,
				[3]=0,
				[4]=1,
			},
		},
		['局外数据']={
			['dbfid']=1652,
			['uid']='CS2_101t',
		},
	},
	{
		['name']=0x10060037,
		['showname']='圣光闪耀',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561a0001,
			['画家']='James Ryman',
			['描述']='为你的英雄恢复5点生命值。',
			['趣味描述']='因为你值得！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130036,
			},
		},
		['局外数据']={
			['dbfid']=54865,
			['uid']='EX1_192',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060038,
		['showname']='心灵视界',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561a0004,
			['画家']='Michael Komarck',
			['描述']='随机复制对手手牌中的一张牌，将其置入你的手牌。',
			['趣味描述']='你刚才做的事情我都看见了。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013003d,
			},
		},
		['局外数据']={
			['dbfid']=1099,
			['uid']='CS2_003',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060039,
		['showname']='真言术：盾',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x561a0003,
			['画家']='Jessica Jung',
			['描述']='使一个随从获得+2生命值。',
			['趣味描述']='额外的保护自然不错，但是这护盾阻碍了视野。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130037,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=613,
			['uid']='CS2_004',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006003a,
		['showname']='神圣惩击',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561a0008,
			['画家']='Steve Ellis',
			['描述']='对一个随从造成3点伤害。',
			['趣味描述']='这和多么虔诚没有关系，每个人都需要时不时有人鞭策一下。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130038,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=279,
			['uid']='CS1_130',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006003b,
		['showname']='暗言术：痛',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561a0009,
			['画家']='Raymond Swanland',
			['描述']='消灭一个攻击力小于或等于3的随从。',
			['趣味描述']='许多侍僧学徒都会施放：“暗言术：怒”，这个法术是它的进阶法术。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130039,
			},
			['卡牌使用目标条件']={
				[1]={n=1,[1] = 'fargboolean_and',
					[2]={
						[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
							[3]={
								[1]=0x10090004,
							},
							[4]={
								[1]='战场',
							},
							[8]=true,
						},
						[2]={n=5,[1] = '卡牌条件_目标属性比较',
							[3]='攻击',
							[4]='当前值',
							[5]='<=',
							[6]=3,
						},
					},
				},
			},
		},
		['局外数据']={
			['dbfid']=1367,
			['uid']='CS2_234',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006003d,
		['showname']='暗言术：灭',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561a0002,
			['画家']='Raymond Swanland',
			['描述']='消灭一个攻击力大于或等于5的随从。',
			['趣味描述']='如果你的法术没有命中，那么你的目标身上会留下一道闪电形状的伤疤。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013003a,
			},
			['卡牌使用目标条件']={
				[1]={n=1,[1] = 'fargboolean_and',
					[2]={
						[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
							[3]={
								[1]=0x10090004,
							},
							[4]={
								[1]='战场',
							},
							[8]=true,
						},
						[2]={n=5,[1] = '卡牌条件_目标属性比较',
							[3]='攻击',
							[4]='当前值',
							[5]='>=',
							[6]=5,
						},
					},
				},
			},
		},
		['局外数据']={
			['dbfid']=1363,
			['uid']='EX1_622',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006003e,
		['showname']='神圣新星',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x561a0007,
			['画家']='Luca Zontini',
			['描述']='对所有敌方随从造成2点伤害，为所有友方角色恢复2点生命值。',
			['趣味描述']='没有得到圣光眷顾的人切忌使用这个法术，它滋润善者，惩罚恶徒。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013003b,
			},
		},
		['局外数据']={
			['dbfid']=841,
			['uid']='CS1_112',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006003f,
		['showname']='精神控制',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x561a0005,
			['画家']='Sean O’Daniels',
			['描述']='获得一个敌方随从的控制权。',
			['趣味描述']='被称为“最有可能让你的对手捶地撞墙的法术。”',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013003e,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='敌方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=8,
			['uid']='CS1_113',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060040,
		['showname']='次级治疗术',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56210001,
			['原画偏移Y']=-12,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]恢复2点生命值。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['战场卡牌指令']=0x1004000a,
			['卡牌效果']={
				[1]=0x1013003c,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=479,
			['uid']='HERO_09bp',
		},
	},
	{
		['name']=0x10060041,
		['showname']='背刺',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x561c0007,
			['原画偏移X']=-5.0,
			['画家']='Michael Sutfin',
			['描述']='对一个未受伤的随从造成2点伤害。',
			['趣味描述']='大叫着“看那儿！”，然后让你的对手背对着你，这一招总是出奇地有效。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013003f,
			},
			['卡牌使用目标条件']={
				[1]={n=1,[1] = 'fargboolean_and',
					[2]={
						[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
							[3]={
								[1]=0x10090004,
							},
							[4]={
								[1]='战场',
							},
							[8]=true,
						},
						[2]={n=1,[1] = '卡牌条件_目标满血',
						},
					},
				},
			},
		},
		['局外数据']={
			['dbfid']=180,
			['uid']='CS2_072',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060042,
		['showname']='影袭',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561c0008,
			['画家']='Frank Cho',
			['描述']='对敌方英雄造成3点伤害。',
			['趣味描述']='尽管这个技能的名字已被改变，但这并不意味着它就不再“邪恶”。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130042,
			},
		},
		['局外数据']={
			['dbfid']=710,
			['uid']='CS2_075',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060043,
		['showname']='致命药膏',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561c0006,
			['画家']='Trevor Jacobs',
			['描述']='使你的武器获得+2攻击力。',
			['趣味描述']='潜行者们都小心翼翼地保守着制毒的秘密，唯恐法师们把毒药用在他们的法术之中。毒性箭？剧毒之雨？毒元素？听上去就很可怕。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130043,
			},
		},
		['局外数据']={
			['dbfid']=459,
			['uid']='CS2_074',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060044,
		['showname']='毒刃',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561c0003,
			['画家']='Alex Garner',
			['描述']='造成1点伤害，抽一张牌。',
			['趣味描述']='毒如鹤顶，刃若清刚；图穷匕现，君命难长。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130045,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=573,
			['uid']='EX1_278',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060045,
		['showname']='闷棍',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561c0004,
			['画家']='Scott Altmann',
			['描述']='将一个敌方随从移回你的对手的手牌。',
			['趣味描述']='“潜行者”的闷棍技术比“盗贼”好很多，他们不用天赋支持，就可以保证百分之百闷棍命中。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013004b,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='敌方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=461,
			['uid']='EX1_581',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060046,
		['showname']='刀扇',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x561c0002,
			['原画偏移X']=-5.0,
			['画家']='Andrew Robinson',
			['描述']='对所有敌方随从造成1点伤害，抽一张牌。',
			['趣味描述']='转瞬即逝的寒光闪过，漫天飞舞的血花绽放。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130046,
			},
		},
		['局外数据']={
			['dbfid']=667,
			['uid']='EX1_129',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060047,
		['showname']='瘟疫使者',
		['卡牌属性']={
			['费用']=4,
			['攻击']=3,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x561c0001,
			['原画偏移X']=-3.0,
			['原画缩放X']=1.05,
			['原画缩放Y']=1.05,
			['画家']='J. Axer',
			['描述']='战吼：使一个友方随从获得剧毒。',
			['趣味描述']='流感的传播元凶。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040004,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130047,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='我方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='获得剧毒。',
			},
		},
		['局外数据']={
			['dbfid']=54863,
			['uid']='EX1_191',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060048,
		['showname']='刺客之刃',
		['卡牌属性']={
			['费用']=5,
			['攻击']=3,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x561c0005,
			['原画缩放X']=0.9,
			['原画缩放Y']=0.9,
			['画家']='Brian Huang',
			['趣味描述']='只有真正的刺客才配拥有。如假包换。',
		},
		['逻辑数据']={
			['类型']=0x10090006,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=421,
			['uid']='CS2_080',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060049,
		['showname']='刺杀',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x561c0009,
			['画家']='Glenn Rane',
			['描述']='消灭一个敌方随从。',
			['趣味描述']='要是你不想被人暗杀，躲到贫瘠之地，然后隐姓埋名。祝你好运！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130049,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='敌方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=345,
			['uid']='CS2_076',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006004a,
		['showname']='疾跑',
		['卡牌属性']={
			['费用']=7,
		},
		['美术数据']={
			['原画']=0x561c000a,
			['画家']='James Zhang',
			['描述']='抽四张牌。',
			['趣味描述']='潜行者们通常都不擅长慢跑。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013004a,
			},
		},
		['局外数据']={
			['dbfid']=630,
			['uid']='CS2_077',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006004b,
		['showname']='匕首精通',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561b0001,
			['原画偏移Y']=-4.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]装备一把1/2的匕首。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['战场卡牌指令']=0x1004000c,
			['卡牌效果']={
				[1]=0x1013004c,
			},
		},
		['局外数据']={
			['dbfid']=730,
			['uid']='HERO_03bp',
		},
	},
	{
		['name']=0x1006004c,
		['showname']='邪恶短刀',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x561b0002,
			['原画缩放X']=0.9,
			['原画缩放Y']=0.9,
		},
		['逻辑数据']={
			['类型']=0x10090006,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=485,
			['uid']='CS2_082',
		},
	},
	{
		['name']=0x1006004d,
		['showname']='先祖治疗',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x561e0009,
			['画家']='Dan Scott',
			['描述']='为一个随从恢复所有生命值并使其获得嘲讽。',
			['趣味描述']='你能在战斗中能够召唤祖先的灵魂！所以，出生在一个强大的家族是很重要的！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013004d,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=149,
			['uid']='CS2_041',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006004e,
		['showname']='图腾之力',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x561e0007,
			['原画偏移X']=-5.0,
			['画家']='Trent Kaniuga',
			['描述']='使你的图腾获得+2生命值。',
			['趣味描述']='禁止踩踏图腾。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013004e,
			},
		},
		['局外数据']={
			['dbfid']=830,
			['uid']='EX1_244',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006004f,
		['showname']='冰霜震击',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561e000a,
			['画家']='Alex Horley Orlandelli',
			['描述']='对一个敌方角色造成1点伤害，并使其冻结。',
			['趣味描述']='年轻的萨满们可能没听说过《你与冰霜震击》这本书。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130050,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='敌方',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=971,
			['uid']='CS2_037',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060050,
		['showname']='石化武器',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561e0004,
			['画家']='Alex Horley Orlandelli',
			['描述']='在本回合中，使一个友方角色获得+3攻击力。',
			['趣味描述']='如果你的敌人是石头做的，那就再好不过了。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130051,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='我方',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=239,
			['uid']='CS2_045',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060051,
		['showname']='风怒',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561e000b,
			['画家']='Justin Sweet',
			['描述']='使一个随从获得风怒。',
			['趣味描述']='比起“地怒”或是“火怒”，萨满们更喜欢使用“风怒”，因为它更轻巧，更飘逸。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130052,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=51,
			['uid']='CS2_039',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060052,
		['showname']='火舌图腾',
		['卡牌属性']={
			['费用']=2,
			['攻击']=0,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x561e0001,
			['原画偏移X']=-5.0,
			['画家']='Jonathan Ryder',
			['描述']='相邻的随从获得+2攻击力。',
			['趣味描述']='图腾制造师喜欢用最稀有的木材来打造图腾。甚至有传言说，这些图腾是由埃隆巴克保护者身上的树皮做的。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['种族']=0x100a0008,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x1013005b,
			},
		},
		['局外数据']={
			['dbfid']=1008,
			['uid']='EX1_565',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060053,
		['showname']='妖术',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x561e0006,
			['画家']='Steve Hui',
			['描述']='使一个随从变形成为一个0/1并具有嘲讽的青蛙。',
			['趣味描述']='如果你对一个鱼人使用妖术...其实结果也没什么变化，对不对？',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130054,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=766,
			['uid']='EX1_246',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060054,
		['showname']='青蛙',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=0,
			['攻击']=0,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x561e0005,
			['原画偏移X']=-5.0,
			['描述']='嘲讽',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=548,
			['uid']='hexfrog',
		},
	},
	{
		['name']=0x10060055,
		['showname']='风语者',
		['卡牌属性']={
			['费用']=4,
			['攻击']=3,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x561e0008,
			['画家']='Vance Kovacs',
			['描述']='战吼：使一个友方随从获得风怒。',
			['趣味描述']='患有口臭的风语者...还有什么比这更糟糕的吗？',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040004,
			['卡牌效果']={
				[1]=0x10130055,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='我方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='使一个随从获得风怒。',
			},
		},
		['局外数据']={
			['dbfid']=178,
			['uid']='EX1_587',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060056,
		['showname']='嗜血',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x561e0003,
			['画家']='Luca Zontini',
			['描述']='在本回合中，使你的随从获得+3攻击力。',
			['趣味描述']='呜啊哇哇哇哇哇！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130057,
			},
		},
		['局外数据']={
			['dbfid']=1171,
			['uid']='CS2_046',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060057,
		['showname']='火元素',
		['卡牌属性']={
			['费用']=6,
			['攻击']=6,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x561e0002,
			['原画偏移X']=-5.0,
			['画家']='Ralph Horsley',
			['描述']='战吼：造成3点伤害。',
			['趣味描述']='他从来不洗澡。嗯...',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['种族']=0x100a0009,
			['卡牌指令']=0x10040004,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130058,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='造成3点伤害。',
			},
		},
		['局外数据']={
			['dbfid']=189,
			['uid']='CS2_042',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060058,
		['showname']='图腾召唤',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561d0002,
			['原画偏移Y']=-12,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]随机召唤一个图腾。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['战场卡牌指令']=0x1004000c,
			['卡牌效果']={
				[1]=0x10130059,
			},
		},
		['局外数据']={
			['dbfid']=687,
			['uid']='HERO_02bp',
		},
	},
	{
		['name']=0x10060059,
		['showname']='灼热图腾',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x561d0001,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['种族']=0x100a0008,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=16222,
			['uid']='AT_132_SHAMANb',
		},
	},
	{
		['name']=0x1006005a,
		['showname']='石爪图腾',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=0,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x561d0004,
			['原画偏移X']=-5.0,
			['描述']='嘲讽',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['种族']=0x100a0008,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=16223,
			['uid']='AT_132_SHAMANc',
		},
	},
	{
		['name']=0x1006005b,
		['showname']='空气之怒图腾',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=0,
			['生命']=2,
			['法术伤害']=1,
		},
		['美术数据']={
			['原画']=0x561d0005,
			['原画偏移X']=-5.0,
			['描述']='法术伤害+1',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['种族']=0x100a0008,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=33554432,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130085,
			},
		},
		['局外数据']={
			['dbfid']=16225,
			['uid']='AT_132_SHAMANd',
		},
	},
	{
		['name']=0x1006005c,
		['showname']='治疗图腾',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=0,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x561d0003,
			['原画偏移X']=-5.0,
			['描述']='在你的回合结束时，为所有友方随从恢复1点生命值。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['种族']=0x100a0008,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x10130086,
			},
		},
		['局外数据']={
			['dbfid']=16221,
			['uid']='AT_132_SHAMANa',
		},
	},
	{
		['name']=0x1006005d,
		['showname']='牺牲契约',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x5620000a,
			['画家']='Jim Nelson',
			['描述']='牺牲一个恶魔，为你的英雄恢复5点生命值。',
			['趣味描述']='这就是为何恶魔不愿意和术士交朋友的原因。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013005a,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[5]={
						[1]=0x100a0007,
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=163,
			['uid']='NEW1_003',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006005e,
		['showname']='死亡缠绕',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56200008,
			['画家']='Matt Gaser',
			['描述']='对一个随从造成1点伤害，如果死亡缠绕将其杀死，抽一张牌。',
			['趣味描述']='很久以前，术士们发现只要在普通的法术中加入一点令人作呕的颜色，就能在伤害敌人的同时使其恐惧。真是一举两得。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013005c,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=1092,
			['uid']='EX1_302',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006005f,
		['showname']='灵魂之火',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56200009,
			['画家']='Raymond Swanland',
			['描述']='造成4点伤害，随机弃一张牌。',
			['趣味描述']='是把别人的灵魂点燃？还是用自己的灵魂把别人点燃？这似乎是有很大区别的。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013005d,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=974,
			['uid']='EX1_308',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060060,
		['showname']='腐蚀术',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56200007,
			['画家']='Wayne Reynolds',
			['描述']='选择一个敌方随从，在你的回合开始时，消灭该随从。',
			['趣味描述']='心灵的腐蚀来自于最初的一些小偷小摸，而在你意识到它之前...嘭！一切化为乌有！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013005e,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='敌方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=982,
			['uid']='CS2_063',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060061,
		['showname']='虚空行者',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x56200003,
			['画家']='Alex Horley Orlandelli',
			['描述']='嘲讽',
			['趣味描述']='虽然他也拥有蓝色的，胖胖的身材，但他和某部动画片里的主角没关系。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=48,
			['uid']='CS2_065',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060062,
		['showname']='魔犬',
		['卡牌属性']={
			['费用']=2,
			['攻击']=4,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x56200001,
			['画家']='Alex Horley',
			['描述']='战吼：随机弃一张牌。',
			['趣味描述']='这个小家伙不咬鞋改咬卡牌了。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x10130060,
			},
		},
		['局外数据']={
			['dbfid']=592,
			['uid']='EX1_306',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060063,
		['showname']='吸取生命',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56200002,
			['画家']='Alex Horley Orlandelli',
			['描述']='造成2点伤害，为你的英雄恢复2点生命值。',
			['趣味描述']='“我只是减少了你一年的寿命而已。”',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130061,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=919,
			['uid']='CS2_061',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060064,
		['showname']='暗影箭',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56200005,
			['画家']='Dave Allsop',
			['描述']='对一个随从造成4点伤害。',
			['趣味描述']='黑暗中的威胁 —— 明枪易躲，暗箭难防。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130062,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=914,
			['uid']='CS2_057',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060065,
		['showname']='地狱烈焰',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x56200004,
			['画家']='Chippy',
			['描述']='对所有角色造成3点伤害。',
			['趣味描述']='鸟尽弓藏，兔死狗烹，术士才不需要什么忠心耿耿的随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x10130063,
			},
		},
		['局外数据']={
			['dbfid']=950,
			['uid']='CS2_062',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060066,
		['showname']='恐惧地狱火',
		['卡牌属性']={
			['费用']=6,
			['攻击']=6,
			['生命']=6,
		},
		['美术数据']={
			['原画']=0x56200006,
			['画家']='Zoltan & Gabor',
			['描述']='战吼：对所有其他角色造成1点伤害。',
			['趣味描述']='“地狱火！”——加拉克苏斯，燃烧军团的艾瑞达之王',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130065,
			},
		},
		['局外数据']={
			['dbfid']=1019,
			['uid']='CS2_064',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060067,
		['showname']='生命分流',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561f0001,
			['原画偏移Y']=0.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]抽一张牌并受到2点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['战场卡牌指令']=0x1004000c,
			['卡牌效果']={
				[1]=0x10130066,
			},
		},
		['局外数据']={
			['dbfid']=300,
			['uid']='HERO_07bp',
		},
	},
	{
		['name']=0x10060068,
		['showname']='冲锋',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56230003,
			['画家']='Alex Horley Orlandelli',
			['描述']='使一个友方随从获得冲锋。在本回合中无法攻击英雄。',
			['趣味描述']='“伙计们！伙计们！你们稳着点儿！”——某些不属于战士的随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x10130068,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='我方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=344,
			['uid']='CS2_103',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060069,
		['showname']='旋风斩',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56230009,
			['画家']='Jonboy Meyers',
			['描述']='对所有随从造成1点伤害。',
			['趣味描述']='分辨资深战士和新手战士的最好办法：新手在用旋风斩时会喊“哇哇哇”。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013006a,
			},
		},
		['局外数据']={
			['dbfid']=636,
			['uid']='EX1_400',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006006a,
		['showname']='斩杀',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56230004,
			['画家']='Dany Orizio',
			['描述']='消灭一个受伤的敌方随从。',
			['趣味描述']='这没什么，他罪有应得。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x1013006b,
			},
			['卡牌使用目标条件']={
				[1]={n=1,[1] = 'fargboolean_and',
					[2]={
						[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
							[2]='敌方',
							[3]={
								[1]=0x10090004,
							},
							[4]={
								[1]='战场',
							},
							[8]=true,
						},
						[2]={n=1,[1] = 'fargboolean_not',
							[2]={n=1,[1] = '卡牌条件_目标满血',
							},
						},
					},
				},
			},
		},
		['局外数据']={
			['dbfid']=785,
			['uid']='CS2_108',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006006b,
		['showname']='英勇打击',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56230001,
			['画家']='Jonboy Meyers',
			['描述']='在本回合中，使你的英雄获得+4攻击力。',
			['趣味描述']='真英雄，必英勇，赶快使用英勇打击，做真英雄！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013006c,
			},
		},
		['局外数据']={
			['dbfid']=1007,
			['uid']='CS2_105',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006006c,
		['showname']='顺劈斩',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56230007,
			['画家']='Phroilan Gardner',
			['描述']='对两个随机敌方随从造成2点伤害。',
			['趣味描述']='嘿，你们俩...你们能不能站在一起？就一会儿...',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013006d,
			},
		},
		['局外数据']={
			['dbfid']=940,
			['uid']='CS2_114',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006006d,
		['showname']='战歌指挥官',
		['卡牌属性']={
			['费用']=3,
			['攻击']=2,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x56230008,
			['画家']='Wei Wang',
			['描述']='你的具有冲锋的随从获得+1攻击力。',
			['趣味描述']='战歌氏族中闹剧不断，什么三教九流的人都能当上他们的指挥官。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x1013006e,
			},
		},
		['局外数据']={
			['dbfid']=1009,
			['uid']='EX1_084',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006006e,
		['showname']='炽炎战斧',
		['卡牌属性']={
			['费用']=3,
			['攻击']=3,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56230002,
			['原画缩放X']=0.9,
			['原画缩放Y']=0.9,
			['画家']='Lucas Graciano',
			['趣味描述']='在安宁祥和的年代，人们用另一个不太流行的名字称呼这把武器，寒冰静谧战斧。',
		},
		['逻辑数据']={
			['类型']=0x10090006,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=401,
			['uid']='CS2_106',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006006f,
		['showname']='盾牌格挡',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56230006,
			['画家']='Michael Komarck',
			['描述']='获得5点护甲值。抽一张牌。',
			['趣味描述']='光有面罩是不够的，所以人们发明了盾牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['卡牌效果']={
				[1]=0x1013006f,
			},
		},
		['局外数据']={
			['dbfid']=1023,
			['uid']='EX1_606',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060070,
		['showname']='库卡隆精英卫士',
		['卡牌属性']={
			['费用']=4,
			['攻击']=4,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x5623000a,
			['原画偏移X']=-5.0,
			['画家']='Alex Horley Orlandelli',
			['描述']='冲锋',
			['趣味描述']='库卡隆就是加尔鲁什·地狱咆哮的精英部队。这么说吧，你最好不要穿着蓝色的战袍出现在这些壮汉面前。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=1,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=28,
			['uid']='NEW1_011',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060071,
		['showname']='奥金斧',
		['卡牌属性']={
			['费用']=5,
			['攻击']=5,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56230005,
			['原画缩放X']=0.9,
			['原画缩放Y']=0.9,
			['画家']='Stefan Kopinski',
			['趣味描述']='“那个消失了的潜行者再也不曾出现过...”',
		},
		['逻辑数据']={
			['类型']=0x10090006,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=304,
			['uid']='CS2_112',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060072,
		['showname']='全副武装！',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56220001,
			['原画偏移Y']=-8.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]获得2点护甲值。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
			['战场卡牌指令']=0x1004000c,
			['卡牌效果']={
				[1]=0x10130067,
			},
		},
		['局外数据']={
			['dbfid']=725,
			['uid']='HERO_01bp',
		},
	},
	{
		['name']=0x10060073,
		['showname']='绵羊',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56150009,
			['原画偏移X']=-31.0,
			['原画偏移Y']=25.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=796,
			['uid']='CS2_tk1',
		},
	},
	{
		['name']=0x1006007a,
		['showname']='暗鳞先知',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x5627002a,
			['原画偏移X']=-5.0,
			['画家']='Phil Saunders',
			['描述']='你的其他鱼人获得+1攻击力。',
			['趣味描述']='这些算是有头脑的鱼人。但仍然有些名不副实。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0002,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x10130071,
			},
		},
		['局外数据']={
			['dbfid']=510,
			['uid']='EX1_508',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006007b,
		['showname']='石牙野猪',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270011,
			['原画偏移X']=-5.0,
			['画家']='Howard Lyon',
			['描述']='冲锋',
			['趣味描述']='决斗的时候离它们远点儿。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=1,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=648,
			['uid']='CS2_171',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006007c,
		['showname']='精灵弓箭手',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270018,
			['画家']='Steve Prescott',
			['描述']='战吼：造成1点伤害。',
			['趣味描述']='媚眼如丝，轻盈如燕，致命如箭。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040004,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130072,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='造成1点伤害。',
			},
		},
		['局外数据']={
			['dbfid']=389,
			['uid']='CS2_189',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006007d,
		['showname']='闪金镇步兵',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x5627002d,
			['原画偏移X']=-5.0,
			['画家']='Donato Giancola',
			['描述']='嘲讽',
			['趣味描述']='如果闪金镇都是由1/2的步兵把守的话，那它早在多年以前就被毁了。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=922,
			['uid']='CS1_042',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006007e,
		['showname']='鱼人袭击者',
		['卡牌属性']={
			['费用']=1,
			['攻击']=2,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270010,
			['画家']='Dan Scott',
			['趣味描述']='姆啦啦咯哈姆噶啊啊嘎，姆啦嘎嘎咯，啵啦啦嗯嘞噶哈，姆啦嗝，咯啦哈哈吧啦咯。加尔鲁什咯嘛啦喝，嘛啦咯呵！',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0002,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=191,
			['uid']='CS2_168',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006007f,
		['showname']='工程师学徒',
		['卡牌属性']={
			['费用']=2,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270022,
			['原画偏移X']=-5.0,
			['画家']='Karl Richardson',
			['描述']='战吼：抽一张牌。',
			['趣味描述']='“班里的一半学生无法毕业...因为他们会被变成小鸡。”——工匠大师欧沃斯巴克，授课于齿轮101班',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130073,
			},
		},
		['局外数据']={
			['dbfid']=284,
			['uid']='EX1_015',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060080,
		['showname']='淡水鳄',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x56270003,
			['原画偏移X']=-5.0,
			['画家']='Daren Bader',
			['趣味描述']='“左撇子”爱德华·史密斯曾经试着从一条淡水鳄的嘴里把行李夺回来。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=1369,
			['uid']='CS2_120',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060081,
		['showname']='狗头人地卜师',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
			['法术伤害']=1,
		},
		['美术数据']={
			['原画']=0x5627000b,
			['原画偏移X']=-5.0,
			['画家']='Gabor Szikszai',
			['描述']='法术伤害+1',
			['趣味描述']='在过去，狗头人是这片土地上最好的蜡烛交易商，直到它们被逼太甚...',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=33554432,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130085,
			},
		},
		['局外数据']={
			['dbfid']=672,
			['uid']='CS2_142',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060082,
		['showname']='蓝腮战士',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270013,
			['原画偏移X']=-5.0,
			['画家']='Jakub Kasper',
			['描述']='冲锋',
			['趣味描述']='他只是想要一个拥抱。一个黏黏的...滑滑的...拥抱。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0002,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=1,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=739,
			['uid']='CS2_173',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060083,
		['showname']='血沼迅猛龙',
		['卡牌属性']={
			['费用']=2,
			['攻击']=3,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56270012,
			['画家']='Dan Brereton',
			['趣味描述']='“去杀30条迅猛龙。”——赫米特·奈辛瓦里',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=216,
			['uid']='CS2_172',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060084,
		['showname']='酸性沼泽软泥怪',
		['卡牌属性']={
			['费用']=2,
			['攻击']=3,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56270026,
			['原画偏移X']=-5.0,
			['画家']='Chris Rahn',
			['描述']='战吼：摧毁对手的武器。',
			['趣味描述']='软泥怪喜欢弗拉曼柯舞。别问为什么。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130074,
			},
		},
		['局外数据']={
			['dbfid']=906,
			['uid']='EX1_066',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060085,
		['showname']='霜狼步兵',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56270004,
			['原画偏移X']=-5.0,
			['画家']='Richie Marella',
			['描述']='嘲讽',
			['趣味描述']='他的祖祖辈辈都是步兵，这已经不仅仅是一份工作而已了。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=41,
			['uid']='CS2_121',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060086,
		['showname']='鱼人猎潮者',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270028,
			['原画偏移X']=-5.0,
			['画家']='Dan Scott',
			['描述']='战吼：召唤一个1/1的鱼人斥候。',
			['趣味描述']='“死亡之潮升涌而起！”',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0002,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130075,
			},
		},
		['局外数据']={
			['dbfid']=976,
			['uid']='EX1_506',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060087,
		['showname']='鱼人斥候',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270029,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0002,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=1078,
			['uid']='EX1_506a',
		},
	},
	{
		['name']=0x10060088,
		['showname']='剃刀猎手',
		['卡牌属性']={
			['费用']=3,
			['攻击']=2,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x56270019,
			['原画偏移X']=-5.0,
			['画家']='Clint Langley',
			['描述']='战吼：召唤一个1/1的野猪。',
			['趣味描述']='有人曾经把塔丝克惹毛过一次。就一次。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130076,
			},
		},
		['局外数据']={
			['dbfid']=257,
			['uid']='CS2_196',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060089,
		['showname']='野猪',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x5627001f,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=298,
			['uid']='CS2_boar',
		},
	},
	{
		['name']=0x1006008a,
		['showname']='团队领袖',
		['卡牌属性']={
			['费用']=3,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56270005,
			['原画偏移X']=-5.0,
			['画家']='Phill Gonzales',
			['描述']='你的其他随从获得+1攻击力。',
			['趣味描述']='“谁再敢OT的，扣掉50DKP！”',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x10130077,
			},
		},
		['局外数据']={
			['dbfid']=1401,
			['uid']='CS2_122',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006008b,
		['showname']='岩浆暴怒者',
		['卡牌属性']={
			['费用']=3,
			['攻击']=5,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270001,
			['原画偏移X']=-5.0,
			['画家']='Matt Gaser',
			['趣味描述']='尽管现在每个人都能单刷熔火之心了，但他依然觉得自己很厉害。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0009,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=1653,
			['uid']='CS2_118',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006008c,
		['showname']='狼骑兵',
		['卡牌属性']={
			['费用']=3,
			['攻击']=3,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270006,
			['原画偏移X']=-5.0,
			['画家']='Dany Orizio',
			['描述']='冲锋',
			['趣味描述']='兽人劫掠者们以狼作为坐骑是因为它们能够适应艰苦的环境，而且它们特别温顺，很会讨兽人喜欢。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=1,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=289,
			['uid']='CS2_124',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006008d,
		['showname']='破碎残阳祭司',
		['卡牌属性']={
			['费用']=3,
			['攻击']=3,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56270023,
			['原画偏移X']=-5.0,
			['画家']='Doug Alexander',
			['描述']='战吼：使一个友方随从获得+1/+1。',
			['趣味描述']='他们会随身带着一瓶太阳井能量饮料！',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040004,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130078,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[2]='我方',
					[3]={
						[1]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='使其获得+1/+1。',
			},
		},
		['局外数据']={
			['dbfid']=608,
			['uid']='EX1_019',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006008e,
		['showname']='达拉然法师',
		['卡牌属性']={
			['费用']=3,
			['攻击']=1,
			['生命']=4,
			['法术伤害']=1,
		},
		['美术数据']={
			['原画']=0x5627002e,
			['原画偏移X']=-5.0,
			['画家']='Jim Nelson',
			['描述']='法术伤害+1',
			['趣味描述']='你肯定没见过达拉然战士。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=33554432,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130085,
			},
		},
		['局外数据']={
			['dbfid']=175,
			['uid']='EX1_582',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006008f,
		['showname']='铁炉堡火枪手',
		['卡牌属性']={
			['费用']=3,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x5627000a,
			['原画偏移X']=-5.0,
			['画家']='Tooth',
			['描述']='战吼：造成1点伤害。',
			['趣味描述']='“准备！瞄准！干杯！”',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040004,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x1013007a,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='造成1点伤害。',
			},
		},
		['局外数据']={
			['dbfid']=339,
			['uid']='CS2_141',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060090,
		['showname']='铁鬃灰熊',
		['卡牌属性']={
			['费用']=3,
			['攻击']=3,
			['生命']=3,
		},
		['美术数据']={
			['原画']=0x56270007,
			['原画偏移X']=-5.0,
			['画家']='Lars Grant-West',
			['描述']='嘲讽',
			['趣味描述']='“瘦死的棕熊比兽人大。”',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=1182,
			['uid']='CS2_125',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060091,
		['showname']='银背族长',
		['卡牌属性']={
			['费用']=3,
			['攻击']=1,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x56270008,
			['原画偏移X']=-5.0,
			['画家']='Daren Bader',
			['描述']='嘲讽',
			['趣味描述']='尽管他表现得像个领导者，但实际上银背母猩猩才是真正管事的人。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=67,
			['uid']='CS2_127',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060092,
		['showname']='侏儒发明家',
		['卡牌属性']={
			['费用']=4,
			['攻击']=2,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x5627000c,
			['原画偏移X']=-5.0,
			['画家']='Court Jones',
			['描述']='战吼：抽一张牌。',
			['趣味描述']='她从来不知道她想发明的是什么，她只知道要发明很棒的东西！',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x1013007b,
			},
		},
		['局外数据']={
			['dbfid']=308,
			['uid']='CS2_147',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060093,
		['showname']='冰风雪人',
		['卡牌属性']={
			['费用']=4,
			['攻击']=4,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x56270015,
			['原画偏移X']=-5.0,
			['画家']='Mauro Cascioli',
			['趣味描述']='他梦想着有一天能够下山开一间拉面店。但他没有那个勇气。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=90,
			['uid']='CS2_182',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060094,
		['showname']='暴风城骑士',
		['卡牌属性']={
			['费用']=4,
			['攻击']=2,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x56270009,
			['原画偏移X']=-5.0,
			['画家']='Ladronn',
			['描述']='冲锋',
			['趣味描述']='对于“死亡之翼毁城事件”，他仍然耿耿于怀。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=1,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=622,
			['uid']='CS2_131',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060095,
		['showname']='机械幼龙技工',
		['卡牌属性']={
			['费用']=4,
			['攻击']=2,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x56270024,
			['原画偏移X']=-5.0,
			['画家']='Warren Mahy',
			['描述']='战吼：召唤一个2/1的机械幼龙。',
			['趣味描述']='她依然在想办法为咬咬先生装上火箭发射器。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x1013007c,
			},
		},
		['局外数据']={
			['dbfid']=523,
			['uid']='EX1_025',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060096,
		['showname']='机械幼龙',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=1,
			['攻击']=2,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270025,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0005,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=59,
			['uid']='EX1_025t',
		},
	},
	{
		['name']=0x10060097,
		['showname']='森金持盾卫士',
		['卡牌属性']={
			['费用']=4,
			['攻击']=3,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x56270014,
			['原画偏移X']=-5.0,
			['画家']='Brian Despain',
			['描述']='嘲讽',
			['趣味描述']='如果你喜欢巨魔和沙尘的话，森金村还是个不错的地方。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=635,
			['uid']='CS2_179',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060098,
		['showname']='绿洲钳嘴龟',
		['卡牌属性']={
			['费用']=4,
			['攻击']=2,
			['生命']=7,
		},
		['美术数据']={
			['原画']=0x56270002,
			['原画偏移X']=-5.0,
			['画家']='Ittoku',
			['趣味描述']='它的梦想是能够像它的偶像那样飞行和吐息火焰，但这样的梦想永远无法实现。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=1370,
			['uid']='CS2_119',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060099,
		['showname']='食人魔法师',
		['卡牌属性']={
			['费用']=4,
			['攻击']=4,
			['生命']=4,
			['法术伤害']=1,
		},
		['美术数据']={
			['原画']=0x5627001a,
			['原画偏移X']=-5.0,
			['画家']='James Ryman',
			['描述']='法术伤害+1',
			['趣味描述']='教授食人魔如何施法是个值得商榷的决定。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=33554432,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130085,
			},
		},
		['局外数据']={
			['dbfid']=995,
			['uid']='CS2_197',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006009a,
		['showname']='古拉巴什狂暴者',
		['卡牌属性']={
			['费用']=5,
			['攻击']=2,
			['生命']=7,
		},
		['美术数据']={
			['原画']=0x56270027,
			['原画偏移X']=-5.0,
			['画家']='Alex Horley Orlandelli',
			['描述']='每当该随从受到伤害，便获得+3攻击力。',
			['趣味描述']='如果那些伤痛没能让你死掉，它们就会让你更强大。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x1013007d,
			},
		},
		['局外数据']={
			['dbfid']=768,
			['uid']='EX1_399',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006009b,
		['showname']='夜刃刺客',
		['卡牌属性']={
			['费用']=5,
			['攻击']=4,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x5627002b,
			['原画偏移X']=-5.0,
			['画家']='Raymond Swanland',
			['描述']='战吼：对敌方英雄造成3点伤害。',
			['趣味描述']='你的脸可能是你最不希望被刀子碰到的地方，但却是潜行者们最喜欢下手的地方。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x1013007f,
			},
		},
		['局外数据']={
			['dbfid']=670,
			['uid']='EX1_593',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006009c,
		['showname']='暗鳞治愈者',
		['卡牌属性']={
			['费用']=5,
			['攻击']=4,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x56270020,
			['原画偏移X']=-5.0,
			['画家']='Jesper Ejsing',
			['描述']='战吼：为所有友方角色恢复2点生命值。',
			['趣味描述']='她只是在闲暇时间做一些治疗工作。这其实只不过是一个业余爱好罢了。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130080,
			},
		},
		['局外数据']={
			['dbfid']=582,
			['uid']='DS1_055',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006009d,
		['showname']='藏宝海湾保镖',
		['卡牌属性']={
			['费用']=5,
			['攻击']=5,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x56270017,
			['原画偏移X']=-5.0,
			['画家']='Matt Cavotta',
			['描述']='嘲讽',
			['趣味描述']='你可以雇佣他们为你效命...直到有人出更高的价钱来让他们对付你。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=1140,
			['uid']='CS2_187',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006009e,
		['showname']='雷矛特种兵',
		['卡牌属性']={
			['费用']=5,
			['攻击']=4,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x5627000d,
			['原画偏移X']=-5.0,
			['画家']='Kev Walker',
			['描述']='战吼：造成2点伤害。',
			['趣味描述']='雷矛特种兵都是爆破专家。他们还会烘焙简易的蛋糕。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040004,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130081,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
			['目标指向文本']={
				[1]='造成2点伤害。',
			},
		},
		['局外数据']={
			['dbfid']=413,
			['uid']='CS2_150',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x1006009f,
		['showname']='霜狼督军',
		['卡牌属性']={
			['费用']=5,
			['攻击']=4,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x5627002c,
			['原画偏移X']=-5.0,
			['画家']='James Ryman',
			['描述']='战吼：战场上每有一个其他友方随从，便获得+1/+1。',
			['趣味描述']='为了争夺奥克兰特山谷的控制权，霜狼氏族和雷矛探险队打得不可开交。任何前来企图进行和平谈判的斡旋者都会被处死。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=2,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130083,
			},
		},
		['局外数据']={
			['dbfid']=496,
			['uid']='CS2_226',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600a0,
		['showname']='大法师',
		['卡牌属性']={
			['费用']=6,
			['攻击']=4,
			['生命']=7,
			['法术伤害']=1,
		},
		['美术数据']={
			['原画']=0x5627000e,
			['原画偏移X']=-5.0,
			['画家']='Steve Ellis',
			['描述']='法术伤害+1',
			['趣味描述']='只要你能消灭任何质疑你的头衔的人，你就能获得“大法师”的头衔。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=0,
				[2]=33554432,
				[3]=0,
				[4]=0,
			},
			['卡牌效果']={
				[1]=0x10130085,
			},
		},
		['局外数据']={
			['dbfid']=525,
			['uid']='CS2_155',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600a1,
		['showname']='石拳食人魔',
		['卡牌属性']={
			['费用']=6,
			['攻击']=6,
			['生命']=7,
		},
		['美术数据']={
			['原画']=0x5627001b,
			['原画偏移X']=-5.0,
			['画家']='Brian Despain',
			['趣味描述']='“我非常厉害，绝对值这个价格！”',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=1686,
			['uid']='CS2_200',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600a2,
		['showname']='竞技场主宰',
		['卡牌属性']={
			['费用']=6,
			['攻击']=6,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x5627000f,
			['原画偏移X']=-5.0,
			['画家']='E.M. Gist',
			['描述']='嘲讽',
			['趣味描述']='他曾经是一个2100分段的竞技场高端玩家。但那已经是几年前的事情了，而且没人能阻止他继续吹嘘这件事。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=2,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=157,
			['uid']='CS2_162',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600a3,
		['showname']='鲁莽火箭兵',
		['卡牌属性']={
			['费用']=6,
			['攻击']=5,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x5627001d,
			['原画偏移X']=-5.0,
			['画家']='John Polidora',
			['描述']='冲锋',
			['趣味描述']='一个疯狂的火箭兵加上一个装满炸药的火箭。完美的组合。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌特性']={
				[1]=1,
				[2]=0,
				[3]=0,
				[4]=0,
			},
		},
		['局外数据']={
			['dbfid']=445,
			['uid']='CS2_213',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600a4,
		['showname']='作战傀儡',
		['卡牌属性']={
			['费用']=7,
			['攻击']=7,
			['生命']=7,
		},
		['美术数据']={
			['原画']=0x56270016,
			['原画偏移X']=-5.0,
			['画家']='Dave Kendall',
			['趣味描述']='傀儡们从来不会胆怯，但是出于某种原因，当你对它们施放“恐惧”的时候，它们依然会逃跑。也许这就是天性？或是想要融入这个世界的渴望？',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=712,
			['uid']='CS2_186',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600a5,
		['showname']='暴风城勇士',
		['卡牌属性']={
			['费用']=7,
			['攻击']=6,
			['生命']=6,
		},
		['美术数据']={
			['原画']=0x5627001e,
			['原画偏移X']=-5.0,
			['画家']='Doug Alexander',
			['描述']='你的其他随从获得+1/+1。',
			['趣味描述']='在死亡之翼攻击主城的时候，这个士兵是小队中唯一活下来的一个。现在，他已经变得忍辱负重，镇定自若了。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040003,
			['卡牌效果']={
				[1]=0x10130084,
			},
		},
		['局外数据']={
			['dbfid']=753,
			['uid']='CS2_222',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600a6,
		['showname']='熔火恶犬',
		['卡牌属性']={
			['费用']=7,
			['攻击']=9,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x5627001c,
			['原画偏移X']=-5.0,
			['画家']='E.M. Gist',
			['趣味描述']='你无法驯服一头熔火恶犬。你只能通过训练让它在吃掉你之前先吃掉其他人。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
			['卡牌指令']=0x10040003,
		},
		['局外数据']={
			['dbfid']=1687,
			['uid']='CS2_201',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600a8,
		['showname']='玛法里奥·怒风',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260001,
			['原画偏移X']=24.0,
			['原画偏移Y']=-5.0,
			['原画缩放X']=1.3,
			['原画缩放Y']=1.3,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=274,
			['uid']='HERO_06',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x1006000f,
			},
		},
	},
	{
		['name']=0x100600a9,
		['showname']='雷克萨',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260002,
			['原画偏移X']=4.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=31,
			['uid']='HERO_05',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x1006001a,
			},
		},
	},
	{
		['name']=0x100600aa,
		['showname']='吉安娜·普罗德摩尔',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260003,
			['原画偏移X']=4.0,
			['原画偏移Y']=-5.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=637,
			['uid']='HERO_08',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x10060028,
			},
		},
	},
	{
		['name']=0x100600ab,
		['showname']='乌瑟尔·光明使者',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260004,
			['原画偏移X']=32.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=671,
			['uid']='HERO_04',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x10060034,
			},
		},
	},
	{
		['name']=0x100600ac,
		['showname']='安度因·乌瑞恩',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x5626000b,
			['原画偏移X']=24.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=813,
			['uid']='HERO_09',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x10060040,
			},
		},
	},
	{
		['name']=0x100600ad,
		['showname']='瓦莉拉·萨古纳尔',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260005,
			['原画偏移X']=5.0,
			['原画偏移Y']=-2.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=930,
			['uid']='HERO_03',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x1006004b,
			},
		},
	},
	{
		['name']=0x100600ae,
		['showname']='萨尔',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260006,
			['原画偏移X']=-11.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=1066,
			['uid']='HERO_02',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x10060058,
			},
		},
	},
	{
		['name']=0x100600af,
		['showname']='古尔丹',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260007,
			['原画偏移X']=-1.0,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=893,
			['uid']='HERO_07',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x10060067,
			},
		},
	},
	{
		['name']=0x100600b0,
		['showname']='加尔鲁什·地狱咆哮',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260008,
			['原画偏移X']=-1.0,
			['原画偏移Y']=16.0,
			['原画缩放X']=1.2,
			['原画缩放Y']=1.2,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['dbfid']=7,
			['uid']='HERO_01',
			['扩展包']=0x100b0001,
			['可收集']=true,
			['衍生卡']={
				[1]=0x10060072,
			},
		},
	},
	{
		['name']=0x100600b1,
		['showname']='雷诺·杰克逊',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x5626001e,
			['原画偏移X']=17.0,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080007,
				[2]=0x10080004,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['扩展包']=0x100b0013,
			['可收集']=true,
			['衍生卡']={
				[1]=0x1006004b,
				[2]=0x10060028,
			},
		},
	},
	{
		['name']=0x100600b2,
		['showname']='芬利爵士',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x5626001d,
			['原画偏移X']=7.0,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080008,
				[2]=0x10080005,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['扩展包']=0x100b0013,
			['可收集']=true,
			['衍生卡']={
				[1]=0x10060058,
				[2]=0x10060034,
			},
		},
	},
	{
		['name']=0x100600b3,
		['showname']='伊莉斯·逐星',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x5626001c,
			['原画偏移X']=-14.0,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080006,
				[2]=0x10080002,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['扩展包']=0x100b0013,
			['可收集']=true,
			['衍生卡']={
				[1]=0x10060040,
				[2]=0x1006000f,
			},
		},
	},
	{
		['name']=0x100600b4,
		['showname']='布莱恩·铜须',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x5626001b,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080003,
				[2]=0x1008000a,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040001,
		},
		['局外数据']={
			['扩展包']=0x100b0013,
			['可收集']=true,
			['衍生卡']={
				[1]=0x1006001a,
				[2]=0x10060072,
			},
		},
	},
	{
		['name']=0x100600b6,
		['showname']='守护者麦迪文',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260009,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080004,
				[2]=0x10080009,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600b7,
		['showname']='安度因国王',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260026,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080006,
				[2]=0x10080005,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600b8,
		['showname']='萨尔大酋长',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260023,
			['原画偏移X']=8.0,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080008,
				[2]=0x1008000a,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600b9,
		['showname']='暗影之王古尔丹',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x5626001f,
			['原画偏移X']=7.0,
			['原画偏移Y']=4.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080008,
				[2]=0x10080009,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x10060aaa,
		['showname']='元气弹',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x562e0001,
			['原画偏移X']=-23.0,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.21,
			['原画缩放Y']=0.78,
			['画家']='鸟山明',
			['描述']='善良的地球人给你力量, 让你可以造成15点伤害。',
			['趣味描述']='不用变成黄毛杀马特就能使用的最强招数',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['卡牌指令']=0x10040002,
			['卡牌效果']={
				[1]=0x101300ee,
			},
			['卡牌使用目标条件']={
				[1]={n=7,[1] = '卡牌条件_目标通用过滤器',
					[3]={
						[1]=0x10090001,
						[2]=0x10090004,
					},
					[4]={
						[1]='战场',
					},
					[8]=true,
				},
			},
		},
		['局外数据']={
			['dbfid']=877,
			['uid']='DS1_185',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600bb,
		['showname']='影蹄杀手',
		['卡牌属性']={
			['费用']=1,
			['攻击']=2,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56330001,
			['原画偏移X']=-5.0,
			['原画偏移Y']=-5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['画家']='A.J. Nazzaro',
			['描述']='战吼：在本回合中，使你的英雄获得+1攻击力。',
			['趣味描述']='在萨格拉斯降临之前，他本是个卑微的精灵弓箭手。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
		},
		['局外数据']={
			['dbfid']=56806,
			['uid']='BT_142',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600bc,
		['showname']='混乱打击',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56330002,
			['画家']='Zoltan Boros',
			['描述']='在本回合中，使你的英雄获得+2攻击力。抽一张牌。',
			['趣味描述']='“我们要什么？”[br]“混乱！”[br]“什么时候要？”[br]“最意外的时候！”',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=56791,
			['uid']='BT_035',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600bd,
		['showname']='盲眼监视者',
		['卡牌属性']={
			['费用']=2,
			['攻击']=3,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56330003,
			['原画偏移X']=-5.0,
			['原画偏移Y']=5.0,
			['画家']='Jaemin Kim',
			['描述']='战吼：检视你牌库中的三张牌。选择一张置于牌库顶。',
			['趣味描述']='他是看不见，但他可以用触手摸。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
		},
		['局外数据']={
			['dbfid']=56973,
			['uid']='BT_323',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600be,
		['showname']='协同打击',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56330004,
			['画家']='L. Lullabi & K. Turovec',
			['描述']='召唤三个1/1并具有突袭的伊利达雷。',
			['趣味描述']='三人行，必有舞师。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=56792,
			['uid']='BT_036',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600bf,
		['showname']='萨特监工',
		['卡牌属性']={
			['费用']=3,
			['攻击']=4,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56330009,
			['原画偏移X']=-5.0,
			['画家']='Zoltan Boros',
			['描述']='在你的英雄攻击后，召唤一个2/2的萨特。',
			['趣味描述']='他是缺乏沟通技巧，但他可以调动快速响应人员。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
		},
		['局外数据']={
			['dbfid']=58489,
			['uid']='BT_352',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600c0,
		['showname']='灵魂裂劈',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56330007,
			['画家']='Zoltan Boros',
			['描述']='吸血[br]随机对两个敌方随从造成2点伤害。',
			['趣味描述']='想抓住恶魔的魂，先抓出它的胃。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=57732,
			['uid']='BT_740',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600c1,
		['showname']='奥达奇战刃',
		['卡牌属性']={
			['费用']=3,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56330006,
			['原画偏移Y']=10.0,
			['原画缩放X']=0.9,
			['原画缩放Y']=0.9,
			['画家']='J. Axer',
			['描述']='吸血',
			['趣味描述']='远古的奥达奇人曾是宇宙中最强的战斗部族。试想如果他们把刀柄老老实实地安在武器的一端，那该有多强。',
		},
		['逻辑数据']={
			['类型']=0x10090006,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=56583,
			['uid']='BT_921',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600c2,
		['showname']='混乱新星',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x5633000b,
			['画家']='Rafael Zanchetin',
			['描述']='对所有随从造成4点伤害。',
			['趣味描述']='伤害输出伊利丹，散播混乱一力担。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=57847,
			['uid']='BT_235',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600c3,
		['showname']='刃缚精锐',
		['卡牌属性']={
			['费用']=5,
			['攻击']=6,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x5633000a,
			['原画偏移X']=-5.0,
			['原画偏移Y']=5.0,
			['画家']='Paul Mafayon',
			['描述']='战吼：在本回合中，如果你的英雄进行过攻击，则造成4点伤害。',
			['趣味描述']='“这是你的战刃，新兵。把它当作你身体的延伸……你刚把它扔出去了？！”',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
			['目标指向文本']={
				[1]='造成4点伤害。',
			},
		},
		['局外数据']={
			['dbfid']=58494,
			['uid']='BT_495',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600c4,
		['showname']='心中的恶魔',
		['卡牌属性']={
			['费用']=8,
		},
		['美术数据']={
			['原画']=0x5633000c,
			['画家']='Gonzalo Ordonez',
			['描述']='在本回合中，使你的英雄获得+8攻击力。',
			['趣味描述']='各种悲情设定的重要组成部分。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=59867,
			['uid']='BT_512',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600d5,
		['showname']='树人',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56140004,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=600,
			['uid']='EX1_158t',
		},
	},
	{
		['name']=0x100600d6,
		['showname']='树人',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56140004,
			['原画偏移X']=-5.0,
			['描述']='嘲讽',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=678,
			['uid']='EX1_573t',
		},
	},
	{
		['name']=0x100600d7,
		['showname']='土狼',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56160006,
			['原画偏移X']=5.0,
			['画家']='Andrew Robinson',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
		},
		['局外数据']={
			['dbfid']=1624,
			['uid']='EX1_534t',
		},
	},
	{
		['name']=0x100600c6,
		['showname']='心灵咒术师',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x561a000b,
			['原画偏移X']=-5.0,
			['画家']='Jim Nelson',
			['描述']='战吼：复制你对手的牌库中的一张牌，并将其置入你的手牌。',
			['趣味描述']='“我看到了……小精灵？你为什么要带小精灵？！”',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=60066,
			['uid']='EX1_193',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600c7,
		['showname']='能量灌注',
		['卡牌属性']={
			['费用']=4,
		},
		['美术数据']={
			['原画']=0x561a0006,
			['画家']='Eva Widermann',
			['描述']='使一个随从获得+2/+6。',
			['趣味描述']='能量灌体，让人雄起，让人欢喜！',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=60067,
			['uid']='EX1_194',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600d8,
		['showname']='游荡小鬼',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x561f0003,
			['原画偏移X']=-5.0,
			['描述']='你的恶魔用完了！但至少...还有小鬼。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
		},
		['局外数据']={
			['dbfid']=1723,
			['uid']='EX1_317t',
		},
	},
	{
		['name']=0x100600d3,
		['showname']='小鸡',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56130003,
			['原画偏移X']=-5.0,
			['描述']='小鸡快跑！',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
		},
		['局外数据']={
			['dbfid']=262,
			['uid']='Mekka4t',
		},
	},
	{
		['name']=0x100600d4,
		['showname']='鱼人',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56270029,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0002,
		},
		['局外数据']={
			['dbfid']=1844,
			['uid']='PRO_001at',
		},
	},
	{
		['name']=0x100600d9,
		['showname']='雏龙',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56130004,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0003,
		},
		['局外数据']={
			['dbfid']=1912,
			['uid']='EX1_116t',
		},
	},
	{
		['name']=0x100600da,
		['showname']='香蕉',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56130005,
			['描述']='使一个随从获得+1/+1。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=1694,
			['uid']='EX1_014t',
		},
	},
	{
		['name']=0x100600db,
		['showname']='小鬼',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56130006,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
		},
		['局外数据']={
			['dbfid']=76,
			['uid']='EX1_598',
		},
	},
	{
		['name']=0x100600dc,
		['showname']='砰砰机器人',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56130007,
			['原画偏移X']=-5.0,
			['描述']='亡语：随机对一个敌人造成1-4点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0005,
		},
		['局外数据']={
			['dbfid']=2235,
			['uid']='GVG_110t',
		},
	},
	{
		['name']=0x100600e4,
		['showname']='蛛魔',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=4,
			['攻击']=4,
			['生命']=4,
		},
		['美术数据']={
			['原画']=0x56130008,
			['原画偏移X']=-5.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=1787,
			['uid']='FP1_007t',
		},
	},
	{
		['name']=0x100600dd,
		['showname']='重型护甲',
		['postfix']='零件',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56320001,
			['画家']='Nutchapol Thitinunthakorn',
			['描述']='使一个随从获得+1生命值。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2151,
			['uid']='PART_001',
		},
	},
	{
		['name']=0x100600de,
		['showname']='时间回溯装置',
		['postfix']='零件',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56320002,
			['画家']='Nutthapon Petthai',
			['描述']='将一个友方随从移回你的手牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2152,
			['uid']='PART_002',
		},
	},
	{
		['name']=0x100600df,
		['showname']='生锈的号角',
		['postfix']='零件',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56320003,
			['画家']='Peerasak Senalai',
			['描述']='使一个随从获得嘲讽。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2153,
			['uid']='PART_003',
		},
	},
	{
		['name']=0x100600e0,
		['showname']='隐秘力场',
		['postfix']='零件',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56320004,
			['画家']='Nutchapol Thitinunthakorn',
			['描述']='直到你的下个回合，使一个友方随从获得潜行。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2154,
			['uid']='PART_004',
		},
	},
	{
		['name']=0x100600e1,
		['showname']='紧急冷冻剂',
		['postfix']='零件',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56320005,
			['画家']='Peerasak Senalai',
			['描述']='冻结一个随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2155,
			['uid']='PART_005',
		},
	},
	{
		['name']=0x100600e2,
		['showname']='形体改造仪',
		['postfix']='零件',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56320006,
			['画家']='Nutthapon Petthai',
			['描述']='使一个随从的攻击力和生命值互换。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2156,
			['uid']='PART_006',
		},
	},
	{
		['name']=0x100600e3,
		['showname']='旋风之刃',
		['postfix']='零件',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56320007,
			['画家']='Nutchapol Thitinunthakorn',
			['描述']='使一个随从获得+1攻击力。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2150,
			['uid']='PART_007',
		},
	},
	{
		['name']=0x100600ba,
		['showname']='伊利丹·怒风',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260027,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['画家']='Alex Horley Orlandelli',
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=56550,
			['uid']='HERO_10',
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600c5,
		['showname']='恶魔之爪',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56310001,
			['原画偏移X']=5.0,
			['原画偏移Y']=-8.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['画家']='Virtuos',
			['描述']='英雄技能[br]在本回合中获得+1攻击力。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=60224,
			['uid']='HERO_10bp',
		},
	},
	{
		['name']=0x100600c8,
		['showname']='恶魔之咬',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56310002,
			['原画偏移X']=5.0,
			['原画偏移Y']=-8.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['画家']='Virtuos',
			['描述']='英雄技能[br]在本回合中获得+2攻击力。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=60483,
			['uid']='HERO_10bp2',
		},
	},
	{
		['name']=0x100600c9,
		['showname']='恐怖变形',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56140003,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]本回合+2攻击力。+2护甲值。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2737,
			['uid']='HERO_06bp2',
		},
	},
	{
		['name']=0x100600ca,
		['showname']='弩炮射击',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56160005,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]对敌方英雄造成3点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080003,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2738,
			['uid']='HERO_05bp2',
		},
	},
	{
		['name']=0x100600cb,
		['showname']='二级火焰冲击',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56180002,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]造成2点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080004,
			},
			['品质']=0x10070001,
			['目标指向文本']={
				[1]='造成2点伤害。',
			},
		},
		['局外数据']={
			['dbfid']=2739,
			['uid']='HERO_08bp2',
		},
	},
	{
		['name']=0x100600cc,
		['showname']='白银之手',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56190003,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]召唤两个1/1的白银之手新兵。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080005,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2740,
			['uid']='HERO_04bp2',
		},
	},
	{
		['name']=0x100600cd,
		['showname']='治疗术',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56210002,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]恢复4点生命值。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080006,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2741,
			['uid']='HERO_09bp2',
		},
	},
	{
		['name']=0x100600ce,
		['showname']='浸毒匕首',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561b0003,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]装备一把2/2的匕首。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2743,
			['uid']='HERO_03bp2',
		},
	},
	{
		['name']=0x100600d2,
		['showname']='浸毒匕首',
		['卡牌属性']={
			['费用']=1,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x561b0003,
			['原画缩放X']=0.9,
			['原画缩放Y']=0.9,
		},
		['逻辑数据']={
			['类型']=0x10090006,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2746,
			['uid']='AT_132_ROGUEt',
		},
	},
	{
		['name']=0x100600cf,
		['showname']='图腾崇拜',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561d0006,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]召唤一个你想要的图腾。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080008,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2742,
			['uid']='HERO_02bp2',
		},
	},
	{
		['name']=0x100600d0,
		['showname']='灵魂分流',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x561f0002,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]抽一张牌。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x10080009,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2744,
			['uid']='HERO_07bp2',
		},
	},
	{
		['name']=0x100600d1,
		['showname']='铜墙铁壁！',
		['卡牌属性']={
			['费用']=2,
		},
		['美术数据']={
			['原画']=0x56220002,
			['原画偏移Y']=-10.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['描述']='英雄技能[br]获得4点护甲值。',
		},
		['逻辑数据']={
			['类型']=0x10090003,
			['职业']={
				[1]=0x1008000a,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=2745,
			['uid']='HERO_01bp2',
		},
	},
	{
		['name']=0x100600e5,
		['showname']='月之女祭司泰兰德',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260028,
			['原画偏移Y']=5.0,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080006,
				[2]=0x10080003,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600e6,
		['showname']='圣法师麦德安',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56260029,
			['原画偏移X']=7.0,
			['原画偏移Y']=-13.0,
			['原画缩放X']=1.3,
			['原画缩放Y']=1.3,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080004,
				[2]=0x10080006,
				[3]=0x10080008,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['扩展包']=0x100b0001,
			['可收集']=true,
		},
	},
	{
		['name']=0x100600e7,
		['showname']='伊利达雷新兵',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['画家']='L. Lullabi & K. Turovec',
			['描述']='突袭',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=56927,
			['uid']='BT_036t',
		},
	},
	{
		['name']=0x100600e8,
		['showname']='伊利达雷萨特',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['画家']='Zoltan Boros',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080022,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
		},
		['局外数据']={
			['dbfid']=58496,
			['uid']='BT_352t',
		},
	},
	{
		['name']=0x100600e9,
		['showname']='泥浆怪',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56130009,
			['原画偏移X']=-5.0,
			['画家']='Anton Kagounkina',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=38921,
			['uid']='OG_202c',
		},
	},
	{
		['name']=0x100600ea,
		['showname']='豺狼人',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x5613000a,
			['原画偏移X']=-5.0,
			['描述']='嘲讽',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=460,
			['uid']='NEW1_040t',
		},
	},
	{
		['name']=0x100600eb,
		['showname']='法力树人',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=2,
			['攻击']=2,
			['生命']=2,
		},
		['美术数据']={
			['原画']=0x56140005,
			['画家']='Matt Gaser',
			['描述']='亡语：获得一个空的法力水晶。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41432,
			['uid']='UNG_111t1',
		},
	},
	{
		['name']=0x100600f8,
		['showname']='迅猛龙',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x5613000b,
			['原画偏移X']=-5.0,
			['画家']='Anton Zemskov',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
		},
		['局外数据']={
			['dbfid']=41248,
			['uid']='UNG_076t1',
		},
	},
	{
		['name']=0x100600f7,
		['showname']='魔暴龙',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=5,
			['攻击']=5,
			['生命']=5,
		},
		['美术数据']={
			['原画']=0x5613000c,
			['原画偏移X']=-10.0,
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
		},
		['局外数据']={
			['dbfid']=332,
			['uid']='EX1_tk29',
		},
	},
	{
		['name']=0x100600ec,
		['showname']='氤氲迷雾',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x5640000b,
			['画家']='Slawomir Maniak',
			['描述']='获得潜行直到你的下个回合。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41054,
			['uid']='UNG_999t10',
		},
	},
	{
		['name']=0x100600ed,
		['showname']='毒液喷吐',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x5640000a,
			['画家']='Mauricio Herrera',
			['描述']='剧毒',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41211,
			['uid']='UNG_999t13',
		},
	},
	{
		['name']=0x100600ee,
		['showname']='火山之力',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56400009,
			['画家']='Rudy Siswanto',
			['描述']='+1/+1',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41691,
			['uid']='UNG_999t14',
		},
	},
	{
		['name']=0x100600ef,
		['showname']='活性孢子',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56400007,
			['画家']='Zoltan Boros',
			['描述']='亡语：召唤两个1/1的植物。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41057,
			['uid']='UNG_999t2',
		},
	},
	{
		['name']=0x100600f0,
		['showname']='烈焰利爪',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56400006,
			['画家']='Garrett Hanna',
			['描述']='+3攻击力',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41058,
			['uid']='UNG_999t3',
		},
	},
	{
		['name']=0x100600f1,
		['showname']='岩质甲壳',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56400005,
			['画家']='Arthur Gimaldinov',
			['描述']='+3生命值',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41059,
			['uid']='UNG_999t4',
		},
	},
	{
		['name']=0x100600f2,
		['showname']='液态外膜',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56400004,
			['画家']='Jakub Kasper',
			['描述']='无法成为法术或英雄技能的目标。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41060,
			['uid']='UNG_999t5',
		},
	},
	{
		['name']=0x100600f3,
		['showname']='巨型体态',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56400003,
			['画家']='Konstantin Turovec',
			['描述']='嘲讽',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41061,
			['uid']='UNG_999t6',
		},
	},
	{
		['name']=0x100600f4,
		['showname']='闪电之速',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56400002,
			['画家']='James Ryman',
			['描述']='风怒',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41062,
			['uid']='UNG_999t7',
		},
	},
	{
		['name']=0x100600f5,
		['showname']='爆裂护盾',
		['postfix']='进化',
		['卡牌属性']={
			['费用']=0,
		},
		['美术数据']={
			['原画']=0x56400001,
			['画家']='L. Lullabi & K. Turovec',
			['描述']='圣盾',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41063,
			['uid']='UNG_999t8',
		},
	},
	{
		['name']=0x100600f6,
		['showname']='植物',
		['postfix']='进化-亡语',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56400008,
			['画家']='Zoltan Boros',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41067,
			['uid']='UNG_999t2t1',
		},
	},
	{
		['name']=0x100600f9,
		['showname']='次级药水',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x5641000a,
			['画家']='Konstantin Turovec',
			['描述']='制造一个法力值消耗为（1）的法术。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41591,
			['uid']='CFM_621t11',
		},
	},
	{
		['name']=0x100600fa,
		['showname']='强效药水',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x5641000b,
			['画家']='Konstantin Turovec',
			['描述']='制造一个法力值消耗为（5）的法术。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41592,
			['uid']='CFM_621t12',
		},
	},
	{
		['name']=0x100600fb,
		['showname']='超级药水',
		['postfix']='衍生卡',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x5641000c,
			['画家']='Konstantin Turovec',
			['描述']='制造一个法力值消耗为（10）的法术。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41593,
			['uid']='CFM_621t13',
		},
	},
	{
		['name']=0x100600fc,
		['showname']='暗金教恶魔',
		['postfix']='卡牌模板',
		['美术数据']={
			['原画']=0x5613000d,
			['原画偏移X']=-5.0,
			['画家']='Slawomir Maniak',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0007,
		},
		['局外数据']={
			['dbfid']=42071,
			['uid']='CFM_621_m4',
		},
	},
	{
		['name']=0x100600fd,
		['showname']='火焰之心',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410001,
			['画家']='Konstantin Turovec',
			['描述']='造成3点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41590,
			['uid']='CFM_621t2',
		},
	},
	{
		['name']=0x100600fe,
		['showname']='邪能花',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410002,
			['画家']='Konstantin Turovec',
			['描述']='对所有随从造成2点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41588,
			['uid']='CFM_621t4',
		},
	},
	{
		['name']=0x100600ff,
		['showname']='冰盖草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410003,
			['画家']='Konstantin Turovec',
			['描述']='随机冻结一个敌方随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41587,
			['uid']='CFM_621t5',
		},
	},
	{
		['name']=0x10060100,
		['showname']='虚空花',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410004,
			['画家']='Konstantin Turovec',
			['描述']='召唤一个2/2的恶魔。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41582,
			['uid']='CFM_621t10',
		},
	},
	{
		['name']=0x10060101,
		['showname']='亡灵腐液',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410005,
			['画家']='Konstantin Turovec',
			['描述']='召唤一个在本局对战中死亡的友方随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=42068,
			['uid']='CFM_621t37',
		},
	},
	{
		['name']=0x10060102,
		['showname']='金棘草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410006,
			['画家']='Konstantin Turovec',
			['描述']='使你的所有随从获得+2生命值。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41586,
			['uid']='CFM_621t6',
		},
	},
	{
		['name']=0x10060103,
		['showname']='石鳞鱼油',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410007,
			['画家']='Konstantin Turovec',
			['描述']='获得4点护甲值。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41589,
			['uid']='CFM_621t3',
		},
	},
	{
		['name']=0x10060104,
		['showname']='皇血草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410008,
			['画家']='Konstantin Turovec',
			['描述']='抽一张牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41584,
			['uid']='CFM_621t8',
		},
	},
	{
		['name']=0x10060105,
		['showname']='暗影之油',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410009,
			['画家']='Konstantin Turovec',
			['描述']='随机将一张恶魔牌置入你的手牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41583,
			['uid']='CFM_621t9',
		},
	},
	{
		['name']=0x10060106,
		['showname']='火焰之心',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x5641000d,
			['画家']='Konstantin Turovec',
			['描述']='造成5点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41605,
			['uid']='CFM_621t16',
		},
	},
	{
		['name']=0x10060107,
		['showname']='神秘羊毛',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x5641000e,
			['画家']='Konstantin Turovec',
			['描述']='随机使一个敌方随从变形成为1/1的绵羊。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41617,
			['uid']='CFM_621t21',
		},
	},
	{
		['name']=0x10060108,
		['showname']='邪能花',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x5641000f,
			['画家']='Konstantin Turovec',
			['描述']='对所有随从造成4点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41607,
			['uid']='CFM_621t18',
		},
	},
	{
		['name']=0x10060109,
		['showname']='冰盖草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56410010,
			['画家']='Konstantin Turovec',
			['描述']='随机冻结两个敌方随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41608,
			['uid']='CFM_621t19',
		},
	},
	{
		['name']=0x1006010a,
		['showname']='虚空花',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56410011,
			['画家']='Konstantin Turovec',
			['描述']='召唤一个5/5的恶魔。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41616,
			['uid']='CFM_621t20',
		},
	},
	{
		['name']=0x1006010b,
		['showname']='亡灵腐液',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56410012,
			['画家']='Konstantin Turovec',
			['描述']='召唤两个在本局对战中死亡的友方随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=42069,
			['uid']='CFM_621t38',
		},
	},
	{
		['name']=0x1006010c,
		['showname']='金棘草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56410013,
			['画家']='Konstantin Turovec',
			['描述']='使你的所有随从获得+4生命值。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41620,
			['uid']='CFM_621t24',
		},
	},
	{
		['name']=0x1006010d,
		['showname']='石鳞鱼油',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56410014,
			['画家']='Konstantin Turovec',
			['描述']='获得7点护甲值。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41606,
			['uid']='CFM_621t17',
		},
	},
	{
		['name']=0x1006010e,
		['showname']='皇血草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56410015,
			['画家']='Konstantin Turovec',
			['描述']='抽两张牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41618,
			['uid']='CFM_621t22',
		},
	},
	{
		['name']=0x1006010f,
		['showname']='暗影之油',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56410016,
			['画家']='Konstantin Turovec',
			['描述']='随机将两张恶魔牌置入你的手牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41619,
			['uid']='CFM_621t23',
		},
	},
	{
		['name']=0x10060110,
		['showname']='神秘羊毛',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x56410017,
			['画家']='Konstantin Turovec',
			['描述']='使所有随从变形成为1/1的绵羊。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41625,
			['uid']='CFM_621t29',
		},
	},
	{
		['name']=0x10060111,
		['showname']='火焰之心',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x56410018,
			['画家']='Konstantin Turovec',
			['描述']='造成8点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41621,
			['uid']='CFM_621t25',
		},
	},
	{
		['name']=0x10060112,
		['showname']='邪能花',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x56410019,
			['画家']='Konstantin Turovec',
			['描述']='对所有随从造成6点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41629,
			['uid']='CFM_621t33',
		},
	},
	{
		['name']=0x10060113,
		['showname']='冰盖草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x5641001a,
			['画家']='Konstantin Turovec',
			['描述']='随机冻结三个敌方随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41623,
			['uid']='CFM_621t27',
		},
	},
	{
		['name']=0x10060114,
		['showname']='虚空花',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x5641001b,
			['画家']='Konstantin Turovec',
			['描述']='召唤一个8/8的恶魔。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41624,
			['uid']='CFM_621t28',
		},
	},
	{
		['name']=0x10060115,
		['showname']='亡灵腐液',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x5641001c,
			['画家']='Konstantin Turovec',
			['描述']='召唤三个在本局对战中死亡的友方随从。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=42070,
			['uid']='CFM_621t39',
		},
	},
	{
		['name']=0x10060116,
		['showname']='金棘草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x5641001d,
			['画家']='Konstantin Turovec',
			['描述']='使你的所有随从获得+6生命值。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41628,
			['uid']='CFM_621t32',
		},
	},
	{
		['name']=0x10060117,
		['showname']='石鳞鱼油',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x5641001e,
			['画家']='Konstantin Turovec',
			['描述']='获得10点护甲值。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41622,
			['uid']='CFM_621t26',
		},
	},
	{
		['name']=0x10060118,
		['showname']='皇血草',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x5641001f,
			['画家']='Konstantin Turovec',
			['描述']='抽三张牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41626,
			['uid']='CFM_621t30',
		},
	},
	{
		['name']=0x10060119,
		['showname']='暗影之油',
		['postfix']='卡扎库斯药水',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x56410020,
			['画家']='Konstantin Turovec',
			['描述']='随机将三张恶魔牌置入你的手牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41627,
			['uid']='CFM_621t31',
		},
	},
	{
		['name']=0x1006011a,
		['showname']='青玉魔像',
		['postfix']='卡牌模板',
		['美术数据']={
			['原画']=0x5613000e,
			['原画偏移X']=-5.0,
			['画家']='Konstantin Turovec',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=42098,
			['uid']='CFM_712_t01',
		},
	},
	{
		['name']=0x1006011b,
		['showname']='青玉魔像',
		['postfix']='卡牌模板',
		['美术数据']={
			['原画']=0x5613000f,
			['原画偏移X']=-5.0,
			['原画偏移Y']=5.0,
			['画家']='Konstantin Turovec',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=42101,
			['uid']='CFM_712_t04',
		},
	},
	{
		['name']=0x1006011c,
		['showname']='青玉魔像',
		['postfix']='卡牌模板',
		['美术数据']={
			['原画']=0x56130010,
			['画家']='Konstantin Turovec',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=42104,
			['uid']='CFM_712_t07',
		},
	},
	{
		['name']=0x1006011d,
		['showname']='青玉魔像',
		['postfix']='卡牌模板',
		['美术数据']={
			['原画']=0x56130011,
			['原画偏移X']=-5.0,
			['原画偏移Y']=5.0,
			['画家']='Konstantin Turovec',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=42128,
			['uid']='CFM_712_t20',
		},
	},
	{
		['name']=0x1006011e,
		['showname']='卡扎库斯药水',
		['postfix']='卡牌模板',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x56410021,
			['画家']='Konstantin Turovec',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41577,
			['uid']='CFM_621t',
		},
	},
	{
		['name']=0x1006011f,
		['showname']='卡扎库斯药水',
		['postfix']='卡牌模板',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56410022,
			['画家']='Konstantin Turovec',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41603,
			['uid']='CFM_621t14',
		},
	},
	{
		['name']=0x10060120,
		['showname']='卡扎库斯药水',
		['postfix']='卡牌模板',
		['卡牌属性']={
			['费用']=10,
		},
		['美术数据']={
			['原画']=0x56410023,
			['画家']='Konstantin Turovec',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=41604,
			['uid']='CFM_621t15',
		},
	},
	{
		['name']=0x10060125,
		['showname']='小精灵',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=0,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56140006,
			['原画偏移X']=5.0,
			['画家']='Jiajun Tian',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080002,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=49149,
			['uid']='GIL_553t',
		},
	},
	{
		['name']=0x10060129,
		['showname']='松鼠',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56130012,
			['原画偏移X']=-5,
			['画家']='Steven Prescott',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0006,
		},
		['局外数据']={
			['dbfid']=54315,
			['uid']='DAL_354t',
		},
	},
	{
		['name']=0x10060128,
		['showname']='海盗杂兵',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56130013,
			['原画偏移X']=-5,
			['画家']='Max Grecke',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['种族']=0x100a0004,
		},
		['局外数据']={
			['dbfid']=50833,
			['uid']='TRL_507t',
		},
	},
	{
		['name']=0x10060132,
		['showname']='融合怪',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56130014,
			['原画偏移X']=-5,
			['画家']='Chris Hayes',
			['描述']='这张牌是元素，机械，恶魔，鱼人，龙，野兽，海盗和图腾。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['种族']=0x100a000a,
		},
		['局外数据']={
			['dbfid']=53019,
			['uid']='DAL_087t',
		},
	},
	{
		['name']=0x10060121,
		['showname']='托林的酒杯',
		['postfix']='宝藏卡',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56570001,
			['画家']='Zoltan Boros',
			['描述']='抽一张牌。将该牌的复制置入你的手牌，直到你的手牌数量达到上限。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=43178,
			['uid']='LOOT_998h',
		},
	},
	{
		['name']=0x10060122,
		['showname']='扎罗格的王冠',
		['postfix']='宝藏卡',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56570002,
			['画家']='Zoltan Boros',
			['描述']='发现一张传说随从牌。召唤两个它的复制。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=43180,
			['uid']='LOOT_998j',
		},
	},
	{
		['name']=0x10060123,
		['showname']='黄金狗头人',
		['postfix']='宝藏卡',
		['卡牌属性']={
			['费用']=3,
			['攻击']=6,
			['生命']=6,
		},
		['美术数据']={
			['原画']=0x56570003,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
			['画家']='Zoltan Boros',
			['描述']='嘲讽，战吼：将你的手牌替换成传说随从。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
			['是精英']=true,
		},
		['局外数据']={
			['dbfid']=45727,
			['uid']='LOOT_998k',
		},
	},
	{
		['name']=0x10060124,
		['showname']='奇迹之杖',
		['postfix']='宝藏卡',
		['卡牌属性']={
			['费用']=3,
		},
		['美术数据']={
			['原画']=0x56570004,
			['画家']='Zoltan Boros',
			['描述']='抽三张牌，其法力值消耗为（0）点。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=45726,
			['uid']='LOOT_998l',
		},
	},
	{
		['name']=0x10060127,
		['showname']='炸弹',
		['postfix']='公用',
		['卡牌属性']={
			['费用']=5,
		},
		['美术数据']={
			['原画']=0x56130015,
			['画家']='Ivan Fomin',
			['描述']='抽到时施放，你受到5点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080001,
			},
		},
		['局外数据']={
			['dbfid']=48752,
			['uid']='BOT_511t',
		},
	},
	{
		['name']=0x1006012a,
		['showname']='无面跟班',
		['postfix']='跟班',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56560001,
			['原画偏移X']=-5,
			['画家']='Slawomir Maniak',
			['描述']='战吼：随机召唤一个法力值消耗为（2）的随从。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
		},
		['局外数据']={
			['dbfid']=53160,
			['uid']='DAL_613',
		},
	},
	{
		['name']=0x1006012b,
		['showname']='狗头人跟班',
		['postfix']='跟班',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56560002,
			['原画偏移X']=-5,
			['画家']='Ivan Fomin',
			['描述']='战吼：造成2点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['目标指向文本']={
				[1]='造成2点伤害。',
			},
		},
		['局外数据']={
			['dbfid']=53161,
			['uid']='DAL_614',
		},
	},
	{
		['name']=0x1006012c,
		['showname']='女巫跟班',
		['postfix']='跟班',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56560003,
			['原画偏移X']=-5,
			['画家']='Jeremy Cranford',
			['描述']='战吼：将一个友方随从变形成为一个法力值消耗增加（1）点的随从。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['目标指向文本']={
				[1]='将一个随从变形。',
			},
		},
		['局外数据']={
			['dbfid']=53162,
			['uid']='DAL_615',
		},
	},
	{
		['name']=0x1006012d,
		['showname']='地精跟班',
		['postfix']='跟班',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56560004,
			['原画偏移X']=-5,
			['画家']='Ivan Fomin',
			['描述']='战吼：使一个友方随从获得+1攻击力和突袭。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['目标指向文本']={
				[1]='获得+1攻击力和突袭。',
			},
		},
		['局外数据']={
			['dbfid']=52897,
			['uid']='DAL_739',
		},
	},
	{
		['name']=0x1006012e,
		['showname']='虚灵跟班',
		['postfix']='跟班',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56560005,
			['原画偏移X']=-5,
			['画家']='Rafael Zanchetin',
			['描述']='战吼：发现一张法术牌。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
		},
		['局外数据']={
			['dbfid']=52900,
			['uid']='DAL_741',
		},
	},
	{
		['name']=0x1006012f,
		['showname']='泰坦造物跟班',
		['postfix']='跟班',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56560006,
			['原画偏移X']=-3.0,
			['画家']='MAR Studio',
			['描述']='战吼：使一个友方随从获得+2生命值和嘲讽。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
			['目标指向文本']={
				[1]='获得+2生命值和嘲讽。',
			},
		},
		['局外数据']={
			['dbfid']=53163,
			['uid']='ULD_616',
		},
	},
	{
		['name']=0x10060130,
		['showname']='龙族跟班',
		['postfix']='跟班',
		['卡牌属性']={
			['费用']=1,
			['攻击']=1,
			['生命']=1,
		},
		['美术数据']={
			['原画']=0x56560007,
			['原画偏移X']=-5,
			['画家']='Dave Allsop',
			['描述']='战吼：发现一张龙牌。',
		},
		['逻辑数据']={
			['类型']=0x10090004,
			['职业']={
				[1]=0x10080001,
			},
		},
		['局外数据']={
			['dbfid']=55378,
			['uid']='DRG_052',
		},
	},
	{
		['name']=0x10060126,
		['showname']='神奇的威兹班',
		['postfix']='卡牌模板',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x5626002a,
			['原画偏移Y']=5,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080021,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=51391,
			['uid']='BOT_914h',
			['扩展包']=0x100b0010,
		},
	},
	{
		['name']=0x10060131,
		['showname']='泽尔，暗影斗篷',
		['postfix']='卡牌模板',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x5626002b,
			['原画偏移Y']=5,
			['原画缩放X']=1.1,
			['原画缩放Y']=1.1,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080021,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=56963,
			['uid']='DAL_800h',
			['扩展包']=0x100b0012,
		},
	},
	{
		['name']=0x10060133,
		['showname']='阿巴瑟',
		['卡牌属性']={
			['生命']=30,
		},
		['美术数据']={
			['原画']=0x56840001,
			['原画偏移X']=-30.0,
			['原画偏移Y']=0.0,
			['原画缩放X']=1.84,
			['原画缩放Y']=1.15,
		},
		['逻辑数据']={
			['类型']=0x10090001,
			['职业']={
				[1]=0x10080023,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['扩展包']=0x100b0001,
		},
	},
	{
		['name']=0x10060134,
		['showname']='皇血草毒素',
		['postfix']='毒素牌',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561b0008,
			['画家']='Tyler Walpole',
			['描述']='抽一张牌。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=38934,
			['uid']='OG_080b',
		},
	},
	{
		['name']=0x10060135,
		['showname']='血蓟毒素',
		['postfix']='毒素牌',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561b0007,
			['画家']='Nutchapol Thitinunthakorn',
			['描述']='将一个友方随从移回你的手牌。它的法力值消耗减少（2）点。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=38938,
			['uid']='OG_080c',
		},
	},
	{
		['name']=0x10060136,
		['showname']='石南草毒素',
		['postfix']='毒素牌',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561b0006,
			['画家']='Matthew O\'Connor',
			['描述']='使一个随从获得+3攻击力。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=38940,
			['uid']='OG_080d',
		},
	},
	{
		['name']=0x10060137,
		['showname']='枯叶草毒素',
		['postfix']='毒素牌',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561b0005,
			['画家']='Joe Wilson',
			['描述']='直到你的下个回合，使一个友方随从获得潜行。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=38942,
			['uid']='OG_080e',
		},
	},
	{
		['name']=0x10060138,
		['showname']='火焰花毒素',
		['postfix']='毒素牌',
		['卡牌属性']={
			['费用']=1,
		},
		['美术数据']={
			['原画']=0x561b0004,
			['画家']='Izzy Hoover',
			['描述']='造成2点伤害。',
		},
		['逻辑数据']={
			['类型']=0x10090005,
			['职业']={
				[1]=0x10080007,
			},
			['品质']=0x10070001,
		},
		['局外数据']={
			['dbfid']=38935,
			['uid']='OG_080f',
		},
	},
}}
return t