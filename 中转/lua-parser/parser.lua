...
local function makeIndexOrCall (t1, t2)
  ...
  return { tag = "Index", tag2 = t2.tag, pos = t1.pos, [1] = t1, [2] = t2[1] }
end
...
function parser.expr(exp)
  local errorinfo = { subject = exp, filename = 'exp' }
  lpeg.setmaxstack(1000)
  G[1] = V"Expr"
  local ast, label, sfail = lpeg.match(G, exp, nil, errorinfo)
  G[1] = V"Lua"
  if not ast then
    local errmsg = labels[label][2]
    print(errmsg)
    return ast, syntaxerror(errorinfo, 0, errmsg)
  end
  return ast
end
